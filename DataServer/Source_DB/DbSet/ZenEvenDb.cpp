...
BOOL CZenEvenDb::SetZen(char* AccountID, int& Zen)
{
	...
	if( m_DBQuery.Fetch() == SQL_NO_DATA)
	{
		...
		{
			Zen = 0;
			LogAdd("[ZEN-EVENT]%s¥‘ %d Zen √ﬂ∞° Ω«∆–", AccountID, Zen);
			return FALSE;
		}

		LogAdd("[ZEN-EVENT]%s¥‘ %d Zen Ω≈√ﬂ∞° √— = %d", AccountID, Zen, Zen+TotalZen);
		return TRUE;
	}
	...
	LogAdd("[ZEN-EVENT](%s)¥‘ %d Zen √ﬂ∞° √— = %d", AccountID, Zen, Zen+TotalZen);
