	...
	while ( true )
	{
		...
		while ( true )
		{
			if ( type == 0 )
			{
				...
				m_i_IT_OpenTime = (int)TokenNumber;

				Token = GetToken();
				m_i_IT_WaitTime = (int)TokenNumber;

				Token = GetToken();
				m_i_IT_PlayTime = (int)TokenNumber;

				Token = GetToken();
				m_i_IT_RestTime = (int)TokenNumber;
			}
			else if ( type == 1 )
			{
				Token = GetToken();
...
BOOL CIllusionTempleEvent::IllusionTempleAddUser(int aIndex, BYTE FloorIndex, BYTE TicketPos)
{
	...
	if(loc8 == 1)
	{
		...
		if(lpObj->pInventory[TicketPos].m_Type == ITEMGET(13,51) )
		{
			...
			GCItemDurSend(lpObj->m_Index, (BYTE)TicketPos, (BYTE)lpObj->pInventory[TicketPos].m_Durability, 0);
		}
...
void CIllusionTempleEvent::EGAnsIllusionTempleEnter(PMSG_ANS_ILLUSIONTEMPLE_ENTER_RESULT * lpMsg)
{
	...
	if(loc8 == 1)
	{
		...
		if(loc4 != 0)
		{
			...
			GCItemDurSend(lpObj->m_Index, (BYTE)lpMsg->iPos, (BYTE)lpObj->pInventory[lpMsg->iPos].m_Durability, 0);
		}
		else
		{
...
BYTE CIllusionTempleEvent::IllusionTempleChaosMix(int arg1, int arg2, int arg3)
{
	...
	if ( (rand()%100) < iMIX_SUCCESS_RATE )	// Success (season 3.0 changed)
	{
		...
		ItemSerialCreateSend(arg1, -1, 0, 0, item_num, (BYTE)arg2, 0, 0, 0, 0, -1, 0, 0);
...
BOOL CIllusionTempleEvent::CheckChoasMixItem(int arg1)
{
	...
	for ( int n=0;n<CHAOS_BOX_SIZE;n++) //loc11
	{
		if ( gObj[arg1].pChaosBox[n].IsItem() == TRUE )
		{
			...
			{
				loc10+= (int)gObj[arg1].pChaosBox[n].m_Durability;
			}
			else
			{
				loc6 = 1; //invalid
...
int CIllusionTempleEvent::CheckLuckyTalisman(int aIndex)
{
	...
	for ( int n=0;n<CHAOS_BOX_SIZE;n++) //loc4
	{
		if ( lpObj->pChaosBox[n].IsItem() == TRUE )
		{
			...
			{
				loc3+= (int)lpObj->pChaosBox[n].m_Durability;
			}
		}
	}
	return loc3;
