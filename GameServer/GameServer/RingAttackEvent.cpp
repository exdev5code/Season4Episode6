...
BOOL CRingMonsterHerd::MonsterHerdItemDrop(LPOBJ lpObj)
{
	if ( lpObj->Class == 135 )
	{
		...
		ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, (BYTE)lpObj->X, (BYTE)lpObj->Y, itemnumber, 0, 0, 0, 0, 0, iIndex, 0, 0);
	...
	if ( lpObj->Class == 136 || lpObj->Class == 137)
	{
		if ( (rand()%100) < 30 )
		{
			...
			ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, (BYTE)lpObj->X, (BYTE)lpObj->Y, itemnumber, 0, 30, 0, 0, 0, iIndex, 0, 0);
			return TRUE;
		}
