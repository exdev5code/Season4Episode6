...
OBJECTSTRUCT*	gObj;
Ex_User ExUser[OBJMAX];
Ex_User2 ExUser2[EXUSERMAX];
int ExIndex = 0;
void SaveExUser(LPOBJ lpObj)
{
	bool AddExIndex = true;
	for(int i=0; i<EXUSERMAX; i++)
	{

		if(!strcmp(ExUser2[i].Name,lpObj->Name))
		{
			ExIndex = i;
			AddExIndex = false;
			break;
...
#ifdef DEBUG_CODE
		LogAddC(2,"ExUser[ExIndex].Name: %s | lpObj->PartyNumber: %d",ExUser[ExIndex].Name, lpObj->PartyNumber);
#endif

	strcpy(ExUser2[ExIndex].Name,lpObj->Name);
	ExUser2[ExIndex].Party = lpObj->PartyNumber;
	ExUser2[ExIndex].TickParty = GetTickCount();
...
void SearchPartyExUSer(LPOBJ lpObj, int &Party, int &PartyTick)
{
	for(int i=0; i<EXUSERMAX; i++)
	{
		if(!strcmp(ExUser2[i].Name,lpObj->Name))
		{
			Party = ExUser2[i].Party;
			PartyTick = ExUser2[i].TickParty;
			return;
		}
	}
}
...
#ifdef TVTEVENT
	TvT.RestoreUser(aIndex);	//TvTEvent
	TvT.RestoreTEvent(aIndex);	//TvTEvent
#endif
#ifdef _LORA_BATTLE_
	LoraBattle.StructStart(aIndex);
#endif
	lpObj->ExCred = 0;
#ifdef PREMIUM_SYSTEM
	ExUser[aIndex].PremiumTime = 0;
#endif
#if __ExGames__
	ExUser[aIndex].ChatBanTime = 0;
#endif
#if __MuRain__
	ExUser[aIndex].NoobTime = 0;
#endif
	ExUser[aIndex].OffExp = 0;
	ExUser[aIndex].OffExpSpeed = 0;
	ExUser[aIndex].OffExpTick = 0;
	ExUser[aIndex].OffExpBuff = 0;

	lpObj->MultiVault = 1;

#ifdef _RECONNECT_
	lpObj->DisconnectType = 0;
#endif

#if(BM_QUEST_SYSTEM_)
	ExUser[aIndex].BMQuest_num = 0;
	ExUser[aIndex].BMQuest_kill = 0;
	ExUser[aIndex].BMQuest_start = 0;
	ExUser[aIndex].BMDamageReflect = 0;
	ExUser[aIndex].BMDamage = 0;
	ExUser[aIndex].BMDefense = 0;
	ExUser[aIndex].BMCriticalDamage = 0;
	ExUser[aIndex].BMExcelentDamage = 0;
	ExUser[aIndex].BMLife = 0;
#endif
...
{
	bool CheckDuel = g_DuelManager.IsOnDuel(aIndex1,aIndex2);
	return CheckDuel;
	/*
...
BOOL gObjSetCharacter(LPBYTE lpdata, int aIndex)
{
...
#ifdef ALL_CHARACTER_CREATE_DEV
	if ( lpObj->Level < 6 || DS_MAP_RANGE(lpObj->MapNumber) != FALSE )
	{
		if ( lpObj->Class == CLASS_ELF )
		{
			lpObj->MapNumber = MAP_INDEX_DEVIAS;//MAP_INDEX_NORIA;
			MapC[lpObj->MapNumber].GetMapPos(lpObj->MapNumber, lpObj->X, lpObj->Y);
		}
		else if ( lpObj->Class == CLASS_SUMMONER )
		{
			lpObj->MapNumber = MAP_INDEX_DEVIAS;//MAP_INDEX_ELBELAND;
			MapC[lpObj->MapNumber].GetMapPos(lpObj->MapNumber, lpObj->X, lpObj->Y);
		}
		else
		{
	...
	ExUser[aIndex].PCPoint = lpMsg->PCPoint;
	ExUser[aIndex].Reset = lpMsg->Reset;
	ExUser[aIndex].GReset = lpMsg->GReset;
	ExUser[aIndex].ExFreePoints = lpMsg->ExFreePoints;	//ExAdd
	ExUser[aIndex].ExQuestNum = lpMsg->ExQuestNum;	//ExAdd
	ExUser[aIndex].ExQuestKill = lpMsg->ExQuestKill;	//ExAdd
	lpObj->ExCred = lpMsg->ExCred;
//#ifdef PREMIUM_SYSTEM
	ExUser[aIndex].PremiumTime = lpMsg->PremiumTime;
//#endif
//#if __ExGames__
	ExUser[aIndex].ChatBanTime = lpMsg->ChatBanTime;
//#endif
//#if __MuRain__
	ExUser[aIndex].NoobTime = lpMsg->NoobTime;
//#endif
//#ifdef _MuReal_
	ExUser[aIndex].OffAfk = lpMsg->OffAfk;
	ExUser[aIndex].VipTime = lpMsg->VipTime;
	ExUser[aIndex].CR = lpMsg->CR;
//#endif
//#ifdef _MegaMu_
	lpObj->ExCred = lpMsg->ExCred;
//#endif
//#if(BM_QUEST_SYSTEM_)
	ExUser[aIndex].BMQuest_num = lpMsg->BMQuest_num;
	ExUser[aIndex].BMQuest_kill = lpMsg->BMQuest_kill;
	ExUser[aIndex].BMQuest_start = lpMsg->BMQuest_start;
	ExUser[aIndex].BMDamageReflect = lpMsg->BMDamageReflect;
	ExUser[aIndex].BMDamage = lpMsg->BMDamage;
	ExUser[aIndex].BMDefense = lpMsg->BMDefense;
	ExUser[aIndex].BMCriticalDamage = lpMsg->BMCriticalDamage;
	ExUser[aIndex].BMExcelentDamage = lpMsg->BMExcelentDamage;
	ExUser[aIndex].BMLife = lpMsg->BMLife;
//#endif

	lpObj->ChaosBank = lpMsg->ChaosBank;
	lpObj->BlessBank = lpMsg->BlessBank;
	lpObj->SoulBank = lpMsg->SoulBank;
	lpObj->LifeBank = lpMsg->LifeBank;

	lpObj->MTDate = lpMsg->MTDate;
	...
	{
		if(ExConfig.CommonServer.MarlonQuestReset)
		{
...
BOOL gObjSetMonster(int aIndex, int MonsterClass)
{
	...
	if(MonsterClass == 479 || MonsterClass == 492)	//Season 4.6 add-on ExTeam
	{
		lpObj->Type = OBJ_NPC;
	}
	//Add ExQuestSystem
	...
	GrandHero.NPCLoad(aIndex, MonsterClass);
#endif

	gCustomNpc.Main(aIndex, MonsterClass);
...
{
	//int Ex_PersonalID = GetPrivateProfileInt("Common","PersonalID",0,"..\\ExTeam\\CommonServer.ini");
	if(!ExConfig.CommonServer.Ex_PersonalID)
		return true;
...
BOOL gObjCheckXYMapTile(LPOBJ lpObj, int iDbgName)
{
	...
	{
#ifndef ALL_CHARACTER_CREATE_DEV
...
void gObjStateProc(LPOBJ lpObj, int aMsgCode, int aIndex, int SubCode)
{
	...
	switch ( aMsgCode )
	{
		...
		case 10:
			...
			{
				gObjAttack(lpObj, &gObj[aIndex], 0, 0, 0, SubCode, 0,1);
			}
			break;
		...
		case 54: //FireScream
			if( lpObj->Live )
			{
				...
				gObjAttack(lpObj, &gObj[aIndex], lpMagic, 1, 0, SubCode, 0,0);
			}
			break;
	...
	//Season 2.5 add-on for third wings
		case 12:
			{
				gObjAttack(lpObj, &gObj[aIndex], 0, 0, 0, SubCode, 0,0);
			}
			break;
		case 13:
			{
	...
	switch ( aMsgCode )
	{
		case 50:
			if ( lpObj->Live != FALSE )
			{
				...
				gObjAttack(lpObj, &gObj[aIndex], lpMagic, 0, 1, 0, SubCode2,0);
			}
			break;
		case 53:
			if( lpObj->Live )
			{
				CMagicInf * lpMagic = gObjGetMagicSearch(lpObj, SubCode);
				gObjAttack(lpObj, &gObj[aIndex], lpMagic, 1, 1, 0, 0,0);
			}
			break;
		case 51:
...
bool gObjLevelUp(LPOBJ lpObj, __int64 & addexp, int iMonsterType, int iEventType) //0051F140
{
	...
	//GJSetCharacterInfo(lpObj, lpObj->m_Index, 0);	//Disable Level Up Save
...
BOOL gObjLevelUpPointAdd(BYTE type, LPOBJ lpObj)
{
	...
	switch ( type )
	{
		case 0:
			{
#ifdef _32K_STATS_
				if((lpObj->Strength+lpObj->AddStrength) >= 32767) return false;
#else
				if((lpObj->Strength+lpObj->AddStrength) >= 65535) return false;
#endif
		...
		case 1:
			{
#ifdef _32K_STATS_
				if((lpObj->Dexterity+lpObj->AddDexterity) >= 32767) return false;
#else
				if((lpObj->Dexterity+lpObj->AddDexterity) >= 65535) return false;
#endif
		...
		case 2:
			{
#ifdef _32K_STATS_
				if((lpObj->Vitality+lpObj->AddVitality) >= 32767) return false;
#else
				if((lpObj->Vitality+lpObj->AddVitality) >= 65535) return false;
#endif
		...
		case 3:
			{
#ifdef _32K_STATS_
				if((lpObj->Energy+lpObj->AddEnergy) >= 32767) return false;
#else
				if((lpObj->Energy+lpObj->AddEnergy) >= 65535) return false;
#endif
		...
		case 4:
			{
#ifdef _32K_STATS_
				if((lpObj->Leadership+lpObj->AddLeadership) >= 32767) return false;
#else
				if((lpObj->Leadership+lpObj->AddLeadership) >= 65535) return false;
#endif
...
#ifdef TVTEVENT
	if(!TvT.PkCheck(lpObj,lpTargetObj))return;	//TvTEvent
#endif 

#ifdef _LORA_BATTLE_
	if(!LoraBattle.CheckPkMap(lpObj,lpTargetObj)) return;
#endif

	if(!PvPMapSystem.CheckPkMap(lpObj,lpTargetObj)) return;
...
void gObjUserDie(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	if(lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER ) 
	{
		if(g_DuelManager.DuelCheck(lpObj, lpTargetObj))
		{
			...
			g_DuelManager.KillUserProc(lpTargetObj,lpObj);
			return;
		}
#ifdef TVTEVENT
		TvT.ProcessKill(lpTargetObj,lpObj);	//TvTEvent
		if(!TvT.PkCheck(lpObj, lpTargetObj))
		{
			return;
		}
#endif
...
#ifdef _LORA_BATTLE_
	LoraBattle.BattleKillSocer(lpTargetObj->m_Index,lpObj->m_Index);	//hz
#endif
...
BOOL gObjWingSprite(LPOBJ lpObj)
{
	...
	if ( (Wing->m_Type >= ITEMGET(12,0) && Wing->m_Type <= ITEMGET(12,6)) || Wing->m_Type <= ITEMGET(13,30) || 
		 ...
		 (Wing->m_Type >= ITEMGET(12,180) && Wing->m_Type <= ITEMGET(12,188)) || //Custom Wings4
#ifdef _WINGS_5LVL_
		 (Wing->m_Type >= ITEMGET(12,189) && Wing->m_Type <= ITEMGET(12,194)) ||
#endif
...
void gObjSpriteDamage(LPOBJ lpObj, int damage)
{
	...
	else if ( sprite->m_Type == ITEMGET(13,80) ) //Panda (Season 4.6 add on)
	{
		...
		if ( sprite->m_Durability < 1.0f )
		{
			...
			if ( BC_MAP_RANGE(lpObj->MapNumber) || CC_MAP_RANGE(lpObj->MapNumber) )
			{
				if ( itemnum == ITEMGET(13,0) || itemnum == ITEMGET(13,1) || 
					itemnum == ITEMGET(13,64) || itemnum == ITEMGET(13,65)) //Season 3.5 add-on
				{
					int pos = -1;
...
void gObjChangeDurProc(LPOBJ lpObj) //#unused in gs 90 now
{
	if(lpObj->pInventory[10].IsItem() == 1 && lpObj->pInventory[10].m_Type == ITEMGET(13,10))
	{
		...
		if(m_Durability > (int)lpObj->pInventory[10].m_Durability)
		{
			...
			GCItemDurSend(lpObj->m_Index,10,(BYTE)dur,0);
	...
	else if(lpObj->pInventory[11].IsItem() == 1 && lpObj->pInventory[11].m_Type == ITEMGET(13,10))
	{
		...
		if(m_Durability > (int)lpObj->pInventory[11].m_Durability)
		{
			...
			GCItemDurSend(lpObj->m_Index,11,(BYTE)dur,0);
	...
	{	
		int dur = (int)lpObj->pInventory[7].m_Durability;
			...
			if( dur > (int)lpObj->pInventory[7].m_Durability ) 
			{
				...
				GCItemDurSend(lpObj->m_Index, 7, (BYTE)send_dur, 0);
				if( send_dur == 0 ) 
				{
	...
	if(DurItem->m_Type != ITEMGET(4,7) && DurItem->m_Type != ITEMGET(4,15))
	{
		if(item_pos < 2)
		{
			if(DurItem->m_Type >= ITEMGET(6,0) && DurItem->m_Type < ITEMGET(7,0))
			{
				...
				if(ret != 0)
				{
					GCItemDurSend2(lpObj->m_Index,(BYTE)item_num[item_pos],(BYTE)DurItem->m_Durability,0);
...
void gObjWeaponDurDown(LPOBJ lpObj, LPOBJ lpTargetObj, int type)
{
	...
	if(type == 0)
	{
		...
		if((lpObj->Class == CLASS_KNIGHT || lpObj->Class == CLASS_MAGUMSA || lpObj->Class == CLASS_DARKLORD) && (Right->m_Type >= 0 && Right->m_Type < ITEMGET(4,0) && Left->m_Type >= 0 && Left->m_Type < ITEMGET(4,0)))
		{
			...
			Left->m_Type == ITEMGET(4,22) || Left->m_Type == ITEMGET(4,23)) //season4.5 add-on
		{
...
void gObjArmorRandomDurDown(LPOBJ lpObj, LPOBJ lpAttackObj)
{
	...
	if(item_pos < 2)
	{
		if(DurItem->m_Type >= ITEMGET(6,0) && DurItem->m_Type < ITEMGET(7,0))
		{
			...
			if(ret != 0)
			{
				GCItemDurSend(lpObj->m_Index,(BYTE)item_num[item_pos],(BYTE)DurItem->m_Durability,0);
				if(ret == 2)
				{
					gObjCalCharacter(lpObj->m_Index);
				}
...
BOOL gObjAttack(LPOBJ lpObj, LPOBJ lpTargetObj, CMagicInf * lpMagic, BOOL magicsend, BYTE MSBFlag, int AttackDamage, BOOL bCombo, BYTE byReflect)
{
	return gclassObjAttack.Attack(lpObj, lpTargetObj, lpMagic, magicsend, MSBFlag, AttackDamage, bCombo, byReflect);
}
...
int gObjMonsterExpSingle(LPOBJ lpObj, LPOBJ lpTargetObj, int dmg, int tot_dmg, bool& bSendExp) //005289E0
{
	...
	lpTargetObj->Money = 0;	//Fix Drop Zen
	if(DS_MAP_RANGE(lpObj->MapNumber))
	{
	...
	if(exp > 0)
	{
		if(lpObj->Type == OBJ_USER)
		{
			...
			gObjPandaUserSetExp(lpObj,exp);	//Panda Use
			gObjUserExperienceMain(lpObj,exp);

#ifdef _SYSTEM_OF_RAGE_
			gSystemOfRage.ExpFunction(lpObj,lpTargetObj);
#endif

			lpObj->Experience += exp;
			lpTargetObj->Money = exp;	//Fix Drop Zen
	...
	//{
	//	//int Ex_PandaExp = GetPrivateProfileInt("Panda","Exp",50,"..\\ExTeam\\Pets.ini");
	//	exp = exp + ( exp * ExConfig.Pet.Ex_PandaExp / 100 );
	//}
	return exp;
}
...
void gObjExpParty(LPOBJ lpObj , LPOBJ lpTargetObj, int AttackDamage, int MSBFlag )
{
...
#ifdef _PARTY_GROUP_
			pres[ipers] = ExUser[number].Reset;
			ipers++;
#endif
	...
	int GroupMin = -1;
	int GroupMax = -1;
	if(pres[0] >= 0 && pres[0] <= 20)
	{
		GroupMin = 0;
		GroupMax = 20;
	}
	else if(pres[0] >= 20 && pres[0] <= 80)
	{
		GroupMin = 20;
		GroupMax = 80;
	}
	...
	for(int j = 0; j < ipers; j++ )
	{
		if( pres[j] >= GroupMin && pres[j] <= GroupMax )
		{
		}
		else
		{
	...
	if(viewplayer > 1)
	{
		if(bApplaySetParty != 0)
		{
			if(viewplayer == 3)
			{
				viewpercent = ExConfig.CommonServer.ExParty3ExpPercent;
			}
			else if(viewplayer == 4)
			{
				viewpercent = ExConfig.CommonServer.ExParty4ExpPercent;
			}
			else if(viewplayer >= 5)
			{
				viewpercent = ExConfig.CommonServer.ExParty5ExpPercent;
			}
			else
			{
				viewpercent = ExConfig.CommonServer.ExPartyExpPercentOther;
			}
		}
		else
		{
			if(viewplayer == 2)
			{
				viewpercent = ExConfig.CommonServer.ExParty2ExpSetPercent;
			}
			else if(viewplayer == 3)
			{
				viewpercent = ExConfig.CommonServer.ExParty3ExpSetPercent;
			}
			else if(viewplayer == 4)
			{
				viewpercent = ExConfig.CommonServer.ExParty4ExpSetPercent;
			}
			else if(viewplayer >= 5)
			{
				viewpercent = ExConfig.CommonServer.ExParty5ExpSetPercent;
			}
			else
			{
				viewpercent = ExConfig.CommonServer.ExParty1ExpSetPercent;
			}
		}
			...
			if(lpTargetObj->MapNumber == lpPartyObj->MapNumber && dis[n] < 10)
			{
				...
				if(exp > 0)
				{
					if(lpPartyObj->Type == OBJ_USER)
					{
						...
						gObjPandaUserSetExp(lpPartyObj,exp);	//Panda Use
						gObjUserExperienceMain(lpPartyObj,exp);
#ifdef _SYSTEM_OF_RAGE_
						gSystemOfRage.ExpFunction(lpPartyObj,lpTargetObj);
#endif
	...
	if(lpTargetObj->Type == OBJ_MONSTER)	//Fix Drop Zen in Party
	{
		lpTargetObj->Money = ExpInZen;	//Fix Drop Zen in Party
	}
}
...
void gObjMonsterDieLifePlus(LPOBJ lpObj, LPOBJ lpTartObj)
{
	...
	{
		lpObj->iShield = (int)((float)lpObj->iShield + (lpObj->iMaxShield + lpObj->iAddShield)/lpObj->m_MLPassiveSkill.m_iML_MonsterAttackSDIncrement);
	}
			...
			if(lpObj->pInventory[n].m_Type == item_type)
			{
				if(lpObj->pInventory[n].m_Level == nLevel)
				{
					if((int)lpObj->pInventory[n].m_Durability + add_dur <= 3)
					{
						...
						GCItemDurSend(lpObj->m_Index,(BYTE)n,(BYTE)lpObj->pInventory[n].m_Durability,0);
						return true;
					}
				}
			}
...
BOOL gObjSearchItemMinus(LPOBJ lpObj, int pos, int m_dur)
{
	...
	if(lpObj->pInventory[n].IsItem() == 1)
	{
		if((int)lpObj->pInventory[n].m_Durability - m_dur > 0)
		{
			...
			GCItemDurSend(lpObj->m_Index,(BYTE)n,(BYTE)lpObj->pInventory[n].m_Durability,1);
			return 1;
		}
	}
	return 0;
...
{
	LogAdd("source = %d, target = %d sFlag = %d tFlag = %d",source,target,sFlag,tFlag);
	LPOBJ lpObj;
	...
	__try
	{
...
#if(_GRAND_HERO_SYSTEM_)
			if(!GrandHero.MoveGRItem(aIndex,sitem->m_Type,target))
			{
				return -1;
			}
#endif
		...
		if(sitem->m_Type != type || 
			...
			sitem->m_Option1 != op1 || 
			sitem->m_Option2 != op2 )
			//sitem->m_Option3 != op3) Fix Chaos Card Master opt 28 add
		{
			...
			if(sFlag == 0)
			{
				...
				else
				{
					if(sitem->m_Level < 4 && sitem->m_Option3*4 < 4)
					{
						...
						else if(sitem->m_Type == ITEMGET(14,208))
						{
		...
		case 4:
			...
			if(ItemMoveBlockExlOpt(sitem->m_NewOption) == FALSE)
			{
				GCServerMsgStringSend("Move Item Block",lpObj->m_Index,1);
				return -1;
			}			
				...
				if(sFlag == 0)
				{
					if(source < INVETORY_WEAR_SIZE)
					{
						if(source == 10 || source == 11)
						{
							...
							if(lpObj->pWarehouse[target].m_Type == ITEMGET(13,76)) //Season 4.6 Panda ring add-on
							{
								gObjUseSkill.SkillChangeUse(aIndex);
							}
						}
				...
				if(sFlag == 0)
				{
					if(source < INVETORY_WEAR_SIZE)
					{
						if(source == 10 || source == 11)
						{
							...
							if(lpObj->pChaosBox[target].m_Type == ITEMGET(13,76)) //Season 4.6 panda ring add-on
							{
								gObjUseSkill.SkillChangeUse(aIndex);
							}
						}
		...
		if(sFlag == 0 && source < INVETORY_WEAR_SIZE)
		{
			if(lpObj->pInventory[source].IsItem() == 1)
			{
				...
				if(lpObj->pInventory[source].m_Type == ITEMGET(13,76)) //Season 4.6 panda ring add-on
				{
					gObjUseSkill.SkillChangeUse(aIndex);
					LogAdd(lMsg.Get(534),gObj[aIndex].Name,lpObj->pInventory[source].m_Level);
				}
			...
			else
			{
				if(source == 10 || source == 11)
				{
					...
					if(lpObj->pInventory[target].m_Type == ITEMGET(13,76)) //Season 4.6 panda add-on
					{
						gObjUseSkill.SkillChangeUse(aIndex);
						LogAdd(lMsg.Get(534),gObj[aIndex].Name,lpObj->pInventory[target].m_Level);
					}
...
BYTE gObjInventoryTradeMove(LPOBJ lpObj, BYTE source, BYTE target)
{
	...
	//Move Item Block to Trade
	if(ItemMoveBlockExlOpt(lpObj->pInventory[source].m_NewOption) == FALSE)
	{
		GCServerMsgStringSend("Move Item Block",lpObj->m_Index,1);
		return -1;
	}
...
void gObjMakePreviewCharSet(int aIndex)
{
	...
	//Custom Wings Fix Visual bug in Select Character
	if ( lpObj->pInventory[7].m_Type >= ITEMGET(12,180) &&  lpObj->pInventory[7].m_Type <= ITEMGET(12,188) )
	{
		lpObj->CharSet[5] |= CS_SET_WING1(3);
		lpObj->CharSet[17] |= (BYTE)(lpObj->pInventory[7].m_Type - 180 + 1) << 2;
	}
	...
	case ITEMGET(13,67): // Season 4.5 addon
		btNewValue = 0x80;
		break;
	case ITEMGET(13,80): // Season 4.6 addon Panda
		btNewValue = 224;
		break;
	}
...
{
	LPOBJ lpObj = &gObj[aIndex];
#if VIESHE_FIX_DC == FALSE

	int DisViewe = ExConfig.CommonServer.DisatanceObjViewe;

	if(x < lpObj->X - DisViewe  || x > lpObj->X + DisViewe || y < lpObj->Y - DisViewe || y > lpObj->Y + DisViewe)	//Object Viewe
	{
		return false;
	}
#else
...
void gObjStateSetCreate(int aIndex)
{
	...
	if(gObjCheckIsInPCBang(lpObj2) != FALSE)
	{
		...
		GCReFillSend(lpObj2->m_Index, (int)(lpObj2->AddLife + lpObj2->MaxLife), 0xFE, 0, (int)(lpObj2->iMaxShield + lpObj2->iAddShield));
		GCManaSend(lpObj2->m_Index, (int)(lpObj2->AddMana + lpObj2->MaxMana), 0xFE, 0, (int)(lpObj2->MaxBP + lpObj2->AddBP));
	}
}
			...
			if(lpObj->DieRegen == 2)
			{
				...
				if(BC_MAP_RANGE(lpObj->MapNumber) && lpObj->Type >= OBJ_MONSTER)
				{
					...
					if(BC_STATUE_RANGE(lpObj->Class - 132) && BC_MAP_RANGE(lpObj->MapNumber))
					{
						...
						ItemSerialCreateSend(lpObj->m_Index,iMapNumber,(BYTE)lpObj->X,(BYTE)lpObj->Y,type,(BYTE)level,0,0,0,0,TopHitUser,0,0);
...
#if(_GRAND_HERO_SYSTEM_)
						else if(GrandHero.HeroMap(lpObj->MapNumber) == true)
						{
							lpObj->MapNumber = MAP_INDEX_RORENCIA;
							MapC[lpObj->MapNumber].GetMapPos(lpObj->MapNumber,(short &)lpObj->X,(short &)lpObj->Y);
						}
...
void gObjSecondProc()
{
	...
	for(n = 0; n < OBJMAX;n++)
	{
		...
		if(lpObj->Connected > PLAYER_LOGGED &&	lpObj->Type == OBJ_USER)
		{
			...
			//OffExp.TickTimes(n);
			ExPCPointSystem.TimeTickPCPoint(n);
#if(_GRAND_HERO_SYSTEM_)
			GrandHero.CheckGRMap(n);
#endif
#ifdef PREMIUM_SYSTEM
			Premium.TickTime(n);
#endif
#if __ExGames__
			if(ExUser[n].ChatBanTime > 0)
			{
				ExUser[n].ChatBanTime--;	//Work ???
			}
#endif
#if __MuRain__
			gNoobSystem.Timer(n);
...
void gObjManaPotionFill(LPOBJ lpObj)
{
	...
	if(lpObj->m_ManaFillCount > 2)
	{
		...
		if(ChangeMana != 0)
		{
			GCManaSend(lpObj->m_Index,(int)lpObj->Mana,255,0,(int)lpObj->BP);
		}
	}
}
...
//////////////////////////////////////////////////////////////////////////////////////////	//TvTEvent
						if(lpObj->m_Change >= 0)
						{
							...
							{
								pViewportCreateChange.DirAndPkLevel |= TvT.ReturnStatus(lpObj->m_Index,tObjNum) & 0x0F;
							}
							else
							{
								pViewportCreateChange.DirAndPkLevel |= lpObj->m_PK_Level & 0x0F;
						...
						else
						{
							...
							if(TvT.CheckStartEvent() == true)	//TvTEvent
							{
								pViewportCreate.DirAndPkLevel |= TvT.ReturnStatus(lpObj->m_Index,tObjNum) & 0x0F;
							}
							else
							{
								pViewportCreate.DirAndPkLevel |= lpObj->m_PK_Level & 0x0F;
...
void gObjSkillUseProc(LPOBJ lpObj)
{
	...
	if(lpObj->Type == OBJ_USER && lpObj->m_iItemEffectValidTime > 0)
	{
		...
		if(lpObj->m_iItemEffectValidTime <= 0)
		{
			...
			GCManaSend(lpObj->m_Index,(int)(lpObj->MaxMana+lpObj->AddMana),0xFE,0,(int)lpObj->MaxBP);
		}
	}
...
BOOL gObjMoveGate(int aIndex, int gt)
{
	...
	int movefail = 0;
#ifdef TVTEVENT
	if(!TvT.CheckGateEvent(aIndex))return false;	//TvTEvent
#endif
...
int	gObjMagicManaUse(LPOBJ lpObj, CMagicInf * lpMagic)
{
	...
	loc2 = (float)MagicDamageC.SkillGetMana(lpMagic->m_Skill);
		...
		if(lpObj->VpPlayer[n].state)
		{
			...
			if(tObjNum >= 0)
			{
				if(gObj[tObjNum].Live)
				{
					...
					{
						if(gObjAttack(lpObj,&gObj[tObjNum],lpMagic,0,0,0,0,0) == 1)
						{
...
BOOL gObjItemLevelUp(LPOBJ lpObj, int source, int target)
{
	...
	if(!( (lpObj->pInventory[target].m_Type < ITEMGET(12,7) ||
		...
		lpObj->pInventory[target].m_Type >=  ITEMGET(12,180)) &&(lpObj->pInventory[target].m_Type <= ITEMGET(12,END_WING_NUM) ||	//Custom Wings4
...
BOOL gObjItemRandomLevelUp(LPOBJ lpObj, int source, int target)
{
	...
	//int Ex_SoulRate = GetPrivateProfileInt("JewelRates","SoulRate",50,"..\\ExTeam\\CommonServer.ini");
	//int Ex_SoulRateLuck = GetPrivateProfileInt("JewelRates","SoulRateLuck",20,"..\\ExTeam\\CommonServer.ini");
	if(source < 0 || source > MAIN_INVENTORY_SIZE -1)
	{
		return false;
	}
	...
	{
		_r -= ExConfig.CommonServer.Ex_SoulRateLuck;	//20
	}
	...
	{
		if(_r < ExConfig.CommonServer.Ex_SoulRate)	//50
		{
			bSuccess = true;
		}
	}
	else
	{
		...
		{
			if(_r < ExConfig.CommonServer.Ex_SoulRate)	//40
			{
				bSuccess = true;
			}
		}
		else if(_r < ExConfig.CommonServer.Ex_SoulRate)	//60
		{
			bSuccess = true;
		}
	}
...
BOOL gObjItemRandomOption3Up(LPOBJ lpObj, int source, int target)
{
	...
	{
		//int Ex_LifeRate = GetPrivateProfileInt("JewelRates","LifeRate",50,"..\\ExTeam\\CommonServer.ini");
		
		if(ExConfig.CommonServer.Ex_LifeRate > _r )
		{
			if( (lpObj->pInventory[target].m_Option3) == FALSE)
			{
				if(lpObj->pInventory[target].m_Type >= ITEMGET(12,36) && lpObj->pInventory[target].m_Type <= ITEMGET(12,40) || lpObj->pInventory[target].m_Type >= ITEMGET(12,180) && lpObj->pInventory[target].m_Type <= ITEMGET(12,END_WING_NUM) || lpObj->pInventory[target].m_Type == ITEMGET(12,43))	//Custom Wings4
				{
...
int gObjCheckAttackArea(int aIndex, int TarObjIndex)
{
	...
	//Fix GSCS ONE Castle Siege Gate Kill
	BYTE attr = 0;
	if(mapnumber != MAP_INDEX_CASTLESIEGE)
	{
...
void gObjUseCircle(int aIndex, int pos)
{
	...
	case 1:
		...
		GCReFillSend(gObj[aIndex].m_Index,(int)(gObj[aIndex].MaxLife + gObj[aIndex].AddLife),0xFE,0,(int)(gObj[aIndex].iMaxShield+gObj[aIndex].iAddShield));
		break;
	case 0:
		gObj[aIndex].Energy += incvalue;
		gObj[aIndex].MaxMana += gObj[aIndex].EnergyToMana * incvalue;
		gObjSetBP(aIndex);
		GCManaSend(gObj[aIndex].m_Index,(int)(gObj[aIndex].MaxMana + gObj[aIndex].AddMana),0xFE,0,(int)gObj[aIndex].MaxBP);
		break;
	default: break;
	}
...
void gObjUsePlusStatFruit(int aIndex,int pos)
{
	...
	GCReFillSend(gObj[aIndex].m_Index,(int)(gObj[aIndex].MaxLife + gObj[aIndex].AddLife),0xFE,0,(int)(gObj[aIndex].iMaxShield + gObj[aIndex].iAddShield));
	gObjSetBP(aIndex);
	GCManaSend(gObj[aIndex].m_Index,(int)(gObj[aIndex].MaxMana + gObj[aIndex].AddMana),0xFE,0,(int)(gObj[aIndex].MaxBP + gObj[aIndex].AddBP));
}
...
void gObjUseCashShopStatFruit(LPOBJ lpObj, int pos)
{
	...
	int iItemDurability = (int)(lpObj->pInventory[pos].m_Durability * 10.0f);
...
void gObjUseCashShopStatPotion(LPOBJ lpObj, int pos)
{
	...
	{
		GCItemDurSend(lpObj->m_Index, (BYTE)pos, (BYTE)lpObj->pInventory[pos].m_Durability,1);
		return;
	}
...
void gObjUseFullRecoveryPotion(LPOBJ lpObj, int pos)
{
	...
	if(lpObj->pInventory[pos].m_Type == ITEMGET(14,70))
	{
		...
		GCReFillSend(lpObj->m_Index,(int)lpObj->Life, -1, 1, (int)lpObj->iShield);
	}
	else
	{
...
void gObjCalcMaxLifePower(int aIndex)
{
	...
	int iLevelBase = lpObj->Level + lpObj->MLevel;
...
{
	//bool bRetVal = g_DuelManager.DuelCheck(lpObj);
	//return bRetVal;
	BOOL bRetVal = FALSE;
...
{
	//bool bRetVal = g_DuelManager.DuelCheck(lpObj,lpTargetObj);
	//return bRetVal;
	BOOL bRetVal = FALSE;
...
void MakeRandomSetItem(int aIndex)
{
	...
	ItemSerialCreateSend(aIndex,(BYTE)gObj[aIndex].MapNumber,(BYTE)gObj[aIndex].X,(BYTE)gObj[aIndex].Y,itemnum,0,0,(BYTE)Option1,(BYTE)Option2,(BYTE)Option3,aIndex,0,(BYTE)SetOption);
...
void MakeRewardSetItem(int aIndex, BYTE cDropX, BYTE cDropY, int iRewardType, int iMapnumber)
{
	...
	ItemSerialCreateSend(aIndex,(BYTE)iMapnumber,(BYTE)cDropX,(BYTE)cDropY,itemnum,0,0,(BYTE)Option1,(BYTE)Option2,(BYTE)Option3,aIndex,0,(BYTE)SetOption);
	...
	for(int iLoop = 0; iLoop < MAIN_INVENTORY_SIZE; iLoop ++)
	{
		...
		if(MAIN_INVENTORY_RANGE(iInventoryIndex))
		{
			...
			else
			{
				...
				GCItemDurSend(iUserIndex,(BYTE)iInventoryIndex,(BYTE)gObj[iUserIndex].pInventory[iInventoryIndex].m_Durability,0);
			}
		}
		else
		{
...
void ExUpdateStats(LPOBJ lpObj, int Stats, int Mode)
{
	...
	switch(Mode)
	{
		case 1: iStatType = FRUIT_STRENGTH; break;		//3
		case 2: iStatType = FRUIT_DEXTERITY; break;	//2
		case 3: iStatType = FRUIT_VITALITY; break;		//1
		case 4: iStatType = FRUIT_ENERGY; break;		//0
		case 5: iStatType = FRUIT_LEADERSHIP; break;	//4
		default: return;
	}
	PMSG_USE_STAT_FRUIT pMsg;
	...
	pMsg.result = STAT_PLUS_SUCCESS; //season 2.5 changed
	pMsg.btFruitType = iStatType;
	pMsg.btStatValue = Stats;
	DataSend(lpObj->m_Index,(LPBYTE)&pMsg,pMsg.h.size);
}
...
{
	ExExperience.Main(lpObj,Experience);
	gMapExp.Main(lpObj,Experience);
#ifdef TALISMAN_SYSTEM
	TalismanSystem.MainExperience(lpObj->m_Index,Experience);
#endif
#ifdef PREMIUM_SYSTEM
	Premium.Exp(lpObj,Experience);
#endif
#if __MuRain__
	gNoobSystem.Experience(lpObj,Experience);
#endif
#ifdef _MuReal_
	VipSystem.VipExperience(lpObj,Experience);
#endif
#ifdef _HAPPY_HOUR_EVENT_
	HappyHour.UserBonusExp(Experience);
#endif
#ifdef _SYSTEM_OF_RAGE_
