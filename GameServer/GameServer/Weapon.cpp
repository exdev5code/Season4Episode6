...
BOOL CWeapon::Attack(LPOBJ lpObj, LPOBJ lpTargetObj, CMagicInf * lpMagic, int iCriticalDamage, int iActionType)
{
	...
	if ( gObjDenorantSprite(lpTargetObj) != FALSE )
	{
		...
		GCReFillSend(lpObj->m_Index, (int)lpObj->Life, (BYTE)-1, 0, (int)lpObj->iShield);
	}
	...
	if ( gObjDarkHorse(lpTargetObj) != FALSE )
	{
		...
		GCReFillSend(lpTargetObj->m_Index, (int)lpTargetObj->Life, (BYTE)-1, 0, (int)lpTargetObj->iShield);
	}
	...
	if ( ManaChange != FALSE )
	{
		GCManaSend(lpTargetObj->m_Index, (int)lpTargetObj->Mana, (BYTE)-1, 0, (int)lpTargetObj->BP);
	}
