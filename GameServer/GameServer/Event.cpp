...
void EventChipOpenEven(LPOBJ lpObj)
{
	...
	if ( (rand()%100) < ::g_EventChipDropRateForBoxOfGold )
	{
		...
		::ItemSerialCreateSend(lpObj->m_Index, (BYTE)lpObj->MapNumber, (BYTE)x, (BYTE)y, type, (BYTE)level, (BYTE)dur, (BYTE)Option1, (BYTE)Option2, (BYTE)Option3, lpObj->m_Index, 0, 0);
	...
	if ( LuckboxItemBag->GetBagCount() > 0 )
	{
		if ( (rand()%20) < ::g_ItemDropRateForBoxOfGold )
		{
			...
			ItemSerialCreateSend(lpObj->m_Index, (BYTE)lpObj->MapNumber, (BYTE)x, (BYTE)y,
				type, (BYTE)level, (BYTE)dur, (BYTE)Option1, (BYTE)Option2, (BYTE)Option3, lpObj->m_Index, (BYTE)ExOption, 0);
...
void SilverMedalOpenEven(LPOBJ lpObj)
{
	...
	if ( SilverMedalItemBag->GetBagCount() > 0 )
	{
		if ( (rand()%20) < g_ItemDropRateForSilverMedal )
		{
			...
			ItemSerialCreateSend(lpObj->m_Index, (BYTE)lpObj->MapNumber, (BYTE)x, (BYTE)y, type, (BYTE)level, (BYTE)dur,
				(BYTE)Option1, (BYTE)Option2, (BYTE)Option3, lpObj->m_Index, 0, 0);
...
void HiddenTreasureBoxItemBoxOpen(LPOBJ lpObj)
{
	...
	if ( (rand()%100) < iItemSuccessRate )
	{
		...
		ItemSerialCreateSend(lpObj->m_Index, (BYTE)lpObj->MapNumber, (BYTE)lpObj->X, (BYTE)lpObj->Y, objTempItem.m_Type, (BYTE)objTempItem.m_Level, (BYTE)objTempItem.m_Durability,
			(BYTE)objTempItem.m_Option1, (BYTE)objTempItem.m_Option2, (BYTE)objTempItem.m_Option3, lpObj->m_Index, (BYTE)objTempItem.m_NewOption, 0);
...
BOOL AttackEvent53BagOpen(LPOBJ lpObj)
{
	...
	if ( Mon53->GetBagCount() > 0 )
	{
		...
		ItemSerialCreateSend(lpObj->m_Index, (BYTE)lpObj->MapNumber, (BYTE)x, (BYTE)y, type, (BYTE)level, (BYTE)dur,
			(BYTE)Option1, (BYTE)Option2, (BYTE)Option3, iMaxHitIndex, 0, 0);
...
void EGRecv2AnvRegSerial( PMSG_ANS_2ANIV_SERIAL* aRecv)
{
	...
	if ( aRecv->btIsRegistered == FALSE )
	{
		...
		{
			ItemSerialCreateSend(gObj[aRecv->iINDEX].m_Index,235,(BYTE)gObj[aRecv->iINDEX].X,(BYTE)gObj[aRecv->iINDEX].Y,ItemNum,(BYTE)level,(BYTE)dur,0,0,0,aRecv->iINDEX,0,0);
