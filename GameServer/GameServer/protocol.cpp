	...
	else
	{
		...
		switch ( protoNum )
		{
			...
			case 0x01:
				...
				break;
			case 0x02:
				CGChatWhisperRecv((PMSG_CHATDATA_WHISPER *)aRecv, aIndex);	//1 vs 1 Chating
				break;
			case 0x03:
...
#if _ANTIHACK_
				gAntiHack.MoveProc((PMSG_MOVE *)aRecv, aIndex);
#endif
				break;
			...
			case 0xD0:
				{
					...
					{
						case 0x05: //buy
							ExPCPointSystem.BuyItem(aIndex, aRecv[4]);
							//CGReqPCBangShopBuy((PMSG_REQ_PCBANG_SHOP_BUY *)aRecv, aIndex);
							break;
						case 0x06: //open
							ExPCPointSystem.OpenShop(aIndex);
...
void CGCheckMainRecv(PMSG_CHECK_MAINEXE_RESULT * lpMsg, int aIndex) 
{
	...
	gObj[aIndex].CheckSumTime = 0;
		...
		return;
#endif
	//int EnableCheckSum = GetPrivateProfileInt("Common","CheckSum",0,"..\\ExTeam\\CommonServer.ini");
	...
	ExPlayerConnectSystem(aIndex);
#ifdef PREMIUM_SYSTEM
	Premium.ConnectInfo(aIndex);
#endif
}
...
void PChatProc(PMSG_CHATDATA * lpChat, short aIndex) 
{
	...
	ChatDataSend(aIndex,(LPBYTE)lpChat);
...
void CSPJoinIdPassRequest(PMSG_IDPASS* lpMsg, int aIndex) 
{
	...
	for(int i = OBJ_MAXMONSTER; i<=OBJMAX;i++)
	{
		...
		{
			if(!strcmp(id,lpObj->AccountID))
			{
				if(ExUser[i].OffTrade != 0)
				{
					GJPUserClose(lpObj->AccountID);
					gObjDel(i);
					ExUser[i].OffTrade = 0;
				}
				else if(ExUser[i].OffExp == 1)
				{
					GJPUserClose(lpObj->AccountID);
					gObjDel(i);
					ExUser[i].OffExp = 0;
...
#if(ENABLE_PROTOCOL_ENG)	
	memcpy(spMsg.Pass, "test", sizeof(spMsg.Pass));
	BuxConvert(spMsg.Pass, MAX_ACCOUNT_LEN);
#else
...
void CGLevelUpPointAdd(PMSG_LVPOINTADD * lpMsg, int aIndex) 
{
	...
	GCReFillSend(aIndex, (int)lpObj->Life, 0xFF, 0, (int)lpObj->iShield);
	GCReFillSend(aIndex, (int)(lpObj->MaxLife + lpObj->AddLife), 0xFE, 0, (int)(lpObj->iMaxShield + lpObj->iAddShield));
	GCManaSend(aIndex, (int)(lpObj->MaxMana + lpObj->AddMana), 0xFE, 0, (int)(lpObj->MaxBP + lpObj->AddBP));
}
		...
		else
		{
					...
					if ( MAIN_INVENTORY_RANGE(pos) )
					{
						if ( MapC[map_num].ItemGive(aIndex, item_num, true) == TRUE )
						{
							...
							if ( gObj[aIndex].pInventory[pos].m_Durability >= 5.0f )
							{
								...
								::ItemSerialCreateSend(aIndex, 235, (BYTE)gObj[aIndex].X, (BYTE)gObj[aIndex].Y, ItemGetNumberMake(14, 28), (BYTE)lpItem->m_Level, 0, 0, 0, 0, aIndex, 0, 0);
			...
			if ( pResult.result != 0xFF )
			{
				ItemByteConvert((LPBYTE)&pResult.Data[0], 
					...
					(BYTE)lpItem->m_SocketOptionIndex);
...
BOOL CGItemDropRequest(PMSG_ITEMTHROW * lpMsg, int aIndex, BOOL drop_type) //004427B0
{
	...
	if ( lpObj->pInventory[lpMsg->Ipos].m_Type >= ITEMGET(12,180) && //Custom Wings4
	     lpObj->pInventory[lpMsg->Ipos].m_Type <= ITEMGET(12,188) &&
		 gPkLimitFree == 0) 
	{
		pResult.Result = false;
	}
...
#ifdef _WINGS_5LVL_
	if ( lpObj->pInventory[lpMsg->Ipos].m_Type >= ITEMGET(12,189) &&
	     lpObj->pInventory[lpMsg->Ipos].m_Type <= ITEMGET(12,194) &&
		 gPkLimitFree == 0) 
	{
		pResult.Result = false;
	}
					...
					case 0:
						{
							...
							ItemSerialCreateSend(aIndex, lpObj->MapNumber, (BYTE)lpObj->X, (BYTE)lpObj->Y, ltype, (BYTE)Level, 1, 0, 0, 0, lpObj->m_Index, 0, 0);
...
void CGInventoryItemMove(PMSG_INVENTORYITEMMOVE * lpMsg, int aIndex) //00446250
{
		 ...
		 (lpMsg->sFlag == 3 && lpMsg->tFlag == 3) )
	{
		...
		if ( DurSSend != FALSE )
			::GCItemDurSend(aIndex, (BYTE)source, (BYTE)lpObj->pInventory[source].m_Durability, FALSE);

		if ( DurTSend != FALSE )
			::GCItemDurSend(aIndex, (BYTE)target, (BYTE)lpObj->pInventory[target].m_Durability, FALSE);
...
void CGBuyRequestRecv(PMSG_BUYREQUEST * lpMsg, int aIndex)
{
	...
	if ( ShopC[tShop].m_item[lpMsg->Pos].IsItem() == TRUE )
	{
...
#if(_ZEN_COIN_==1)
		int MinusZen = 0;
		if(ShopC[tShop].m_item[lpMsg->Pos].m_Type == ITEMGET(14,221))
		{
			MinusZen += 100000000;
		}
		else if(ShopC[tShop].m_item[lpMsg->Pos].m_Type == ITEMGET(14,222))
		{
			MinusZen += 500000000;
		}
		else if(ShopC[tShop].m_item[lpMsg->Pos].m_Type == ITEMGET(14,223))
		{
			MinusZen += 1000000000;
		}
		if(MinusZen>0)
		{
			iStoreTaxMoney = MinusZen;
			if ( lpObj->Money < iStoreTaxMoney )
			{
				MsgNormal(aIndex,"[Zen Coin] Need %d Zen",iStoreTaxMoney);
				DataSend(aIndex, (LPBYTE)&pResult, pResult.h.size);
				return;
			}
		}
		...
		else
		{
			...
			{
				int dur = (int)(ShopC[tShop].m_item[lpMsg->Pos].m_Durability);
...
void CGSellRequestRecv(PMSG_SELLREQUEST * lpMsg, int aIndex) 
{
	...
	//Move Item Blok
	if ( ItemMoveBlockExlOpt(lpObj->pInventory[lpMsg->Pos].m_NewOption) == FALSE )
	{
	...
	else
	{
		...
		PriceManager.SellPriceItem(aIndex,lpMsg->Pos,iAddZen);

#if(_CREDIT_DONATE_)
		gCreditDonate.Sell(aIndex,lpMsg->Pos,iAddZen);
#endif

#if(_CR_COIN_==1)
		int PlusCred = 0;
		if(lpObj->pInventory[lpMsg->Pos].m_Type == ITEMGET(14,215))
		{
			PlusCred++;
		}
		else if(lpObj->pInventory[lpMsg->Pos].m_Type == ITEMGET(14,216))
		{
			PlusCred += 5;
		}
		else if(lpObj->pInventory[lpMsg->Pos].m_Type == ITEMGET(14,217))
		{
			PlusCred += 10;
		}
		else if(lpObj->pInventory[lpMsg->Pos].m_Type == ITEMGET(14,218))
		{
			PlusCred += 20;
		}
		else if(lpObj->pInventory[lpMsg->Pos].m_Type == ITEMGET(14,219))
		{
			PlusCred += 50;
		}
		else if(lpObj->pInventory[lpMsg->Pos].m_Type == ITEMGET(14,220))
		{
			PlusCred += 100;
		}
		if(PlusCred>0)
		{
			iAddZen = 0;
			lpObj->ExCred += PlusCred;
			ExPCPointSystem.InitPCPointForPlayer(lpObj, ExUser[aIndex].PCPoint, lpObj->ExCred );
		}
#endif
#if(_ZEN_COIN_==1)
		int PlusZen = 0;
		if(lpObj->pInventory[lpMsg->Pos].m_Type == ITEMGET(14,221))
		{
			PlusZen += 100000000;
		}
		else if(lpObj->pInventory[lpMsg->Pos].m_Type == ITEMGET(14,222))
		{
			PlusZen += 500000000;
		}
		else if(lpObj->pInventory[lpMsg->Pos].m_Type == ITEMGET(14,223))
		{
			PlusZen += 1000000000;
		}
		if(PlusZen>0)
...
void ItemDurRepaire(LPOBJ lpObj, CItem * DurItem, int pos, int RequestPos) 
{
	...
	if ( itemtype >= ITEMGET(14,0) || ( itemtype >= ITEMGET(13,0) &&itemtype  < ITEMGET(13,4) ) || 
		itemtype == ITEMGET(13,10) || (itemtype >= ITEMGET(12,7) && 
		itemtype < ITEMGET(12,36)) || (itemtype > ITEMGET(12,43) && 
		itemtype < ITEMGET(12,180))|| (itemtype > ITEMGET(12,END_WING_NUM) &&	//Custom Wings4
		itemtype <ITEMGET(13,0) ) ||
	...
	//No Repain Panda Ring
	if ( itemtype == ITEMGET(13,76) )
	{
		pResult.Money = 0;
		DataSend(lpObj->m_Index, (LPBYTE)&pResult, pResult.h.size);
		return;
	...
	else
	{
		...
		GCItemDurSend(lpObj->m_Index, (BYTE)pos, (BYTE)DurItem->m_Durability, FALSE);
	}
	...
	if ( lpMsg->Position == 0xFF )
	{
		for ( int n=0;n<MAIN_INVENTORY_SIZE;n++)
		{
			if ( lpObj->pInventory[n].IsItem())
			{
				...
				if ( lpObj->pInventory[n].m_Type == ITEMGET(13,70) ) //season 4.0 add-on
					continue;
...
void CGPShopReqSetItemPrice(PMSG_REQ_PSHOP_SETITEMPRICE * lpMsg, int aIndex) 
{
	...
	{
		MsgOutput(aIndex, lMsg.Get(MSGGET(13, 26)));
		GCReFillSend(aIndex, (int)gObj[aIndex].Life, 0xFD, 1, (int)gObj[aIndex].iShield);
	...
	{
		GCReFillSend(aIndex, (int)gObj[aIndex].Life, 0xFD, TRUE, (int)gObj[aIndex].iShield);
...
void CGPShopReqBuyItem(PMSG_REQ_BUYITEM_FROM_PSHOP * lpMsg, int aSourceIndex) 
{
	...
	else
	{
		...
		{
			MsgOutput(lpObj->m_Index, lMsg.Get(MSGGET(13, 26)));
			GCReFillSend(aSourceIndex, (int)gObj[aSourceIndex].Life, 0xFD, 1, (int)gObj[aSourceIndex].iShield);
		...
		{
			::GCReFillSend(aSourceIndex, (int)gObj[aSourceIndex].Life, -3, 1, (int)gObj[aSourceIndex].iShield);
			LogAdd("error-L2 : CopyItem [%s][%s] return %s %d",
				gObj[aSourceIndex].AccountID, 
				gObj[aSourceIndex].Name,
				__FILE__, 
		...
		{
			GCReFillSend(lpObj->m_Index, (int)lpObj->Life, -3, 1, (int)lpObj->iShield);
			LogAdd("error-L2 : CopyItem [%s][%s] return %s %d",
				lpObj->AccountID, 
				lpObj->Name,
				__FILE__, 
		...
		__try
		{
			...
			if ( lpObj->Inventory1[lpMsg->btItemPos].IsItem() == TRUE )
			{
				...
				//		ExPoint Shop
				//--------------------------------
				if(!strcmp(lpObj->m_szPShopText,".::EXPOINT SHOP::."))
				{
					if ( ExUser[aSourceIndex].PCPoint < lpObj->Inventory1[lpMsg->btItemPos].m_iPShopValue )
					{
						LogAddTD("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Lack of PCPOINT",
							gObj[aSourceIndex].AccountID, 
							gObj[aSourceIndex].Name, 
							lpObj->AccountID, 
							lpObj->Name);
						//::CGPShopAnsBuyItem(aSourceIndex, lpObj->m_Index, 0, 7);
						MsgNormal(aSourceIndex,"[OffTrade] Need %d ExPoint",lpObj->Inventory1[lpMsg->btItemPos].m_iPShopValue);
						return;
					}
				}
				else if(!strcmp(lpObj->m_szPShopText,".::CREDIT SHOP::."))
				{
					if ( gObj[aSourceIndex].ExCred < lpObj->Inventory1[lpMsg->btItemPos].m_iPShopValue )
					{
						LogAddTD("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Lack of CREDIT",
							...
							lpObj->Name);
						//::CGPShopAnsBuyItem(aSourceIndex, lpObj->m_Index, 0, 7);
						MsgNormal(aSourceIndex,"[OffTrade] Need %d Credit",lpObj->Inventory1[lpMsg->btItemPos].m_iPShopValue);
						return;
					}
				}
				//--------------------------------
				//		WCoin Shop
				//--------------------------------
				else if(!strcmp(lpObj->m_szPShopText,".::WCOINT SHOP::."))
				{
					if ( gObj[aSourceIndex].m_wCashPoint < lpObj->Inventory1[lpMsg->btItemPos].m_iPShopValue )
					{
						LogAddTD("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Lack of Wcoin",
							...
							lpObj->Name);
						//::CGPShopAnsBuyItem(aSourceIndex, lpObj->m_Index, 0, 7);
						MsgNormal(aSourceIndex,"[OffTrade] Need %d WCoin",lpObj->Inventory1[lpMsg->btItemPos].m_iPShopValue);
						return;
					}
				}
				else if(ExUser[lpObj->m_Index].OffTrade == OFFCHAOS_DC)
				{
					if ( gObj[aSourceIndex].ChaosBank < lpObj->Inventory1[lpMsg->btItemPos].m_iPShopValue )
					{
						LogAddTD("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Lack of Chaos Bank",
							...
							lpObj->AccountID, 
							lpObj->Name);
						MsgNormal(aSourceIndex,"[OffTrade] Need %d Chaos Bank",lpObj->Inventory1[lpMsg->btItemPos].m_iPShopValue);
						return;
					}
				}
				else if(ExUser[lpObj->m_Index].OffTrade == OFFBLESS_DC)
				{
					if ( gObj[aSourceIndex].BlessBank < lpObj->Inventory1[lpMsg->btItemPos].m_iPShopValue )
					{
						LogAddTD("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Lack of Bless Bank",
							gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name, lpObj->AccountID, lpObj->Name);
						MsgNormal(aSourceIndex,"[OffTrade] Need %d Bless Bank",lpObj->Inventory1[lpMsg->btItemPos].m_iPShopValue);
						return;
					}
				}
				else if(ExUser[lpObj->m_Index].OffTrade == OFFSOUL_DC)
				{
					if ( gObj[aSourceIndex].SoulBank < lpObj->Inventory1[lpMsg->btItemPos].m_iPShopValue )
					{
						LogAddTD("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Lack of Soul Bank",
							gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name, lpObj->AccountID, lpObj->Name);
						MsgNormal(aSourceIndex,"[OffTrade] Need %d Soul Bank",lpObj->Inventory1[lpMsg->btItemPos].m_iPShopValue);
						return;
					}
				}
				else if(ExUser[lpObj->m_Index].OffTrade == OFFLIFE_DC)
				{
					if ( gObj[aSourceIndex].LifeBank < lpObj->Inventory1[lpMsg->btItemPos].m_iPShopValue )
					{
						LogAddTD("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Lack of Life Bank",
							gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name, lpObj->AccountID, lpObj->Name);
						MsgNormal(aSourceIndex,"[OffTrade] Need %d Life Bank",lpObj->Inventory1[lpMsg->btItemPos].m_iPShopValue);
						return;
					}
				}
				else
				...
				//	OffTrade
				if(strcmp(lpObj->m_szPShopText,".::EXPOINT SHOP::.") && strcmp(lpObj->m_szPShopText,".::EXPOINT SHOP::."))
				{
				...
				if(!strcmp(lpObj->m_szPShopText,".::EXPOINT SHOP::."))
				{
					ExUser[aSourceIndex].PCPoint -= dwCost;
					ExUser[lpObj->m_Index].PCPoint += dwCost;
					ExPCPointSystem.InitPCPointForPlayer(&gObj[aSourceIndex],ExUser[aSourceIndex].PCPoint,gObj[aSourceIndex].ExCred);
					ExPCPointSystem.InitPCPointForPlayer(lpObj,ExUser[lpObj->m_Index].PCPoint,lpObj->ExCred);		//TEST
				}
				else if(!strcmp(lpObj->m_szPShopText,".::CREDIT SHOP::."))
				{
					gObj[aSourceIndex].ExCred -= dwCost;
					gObj[lpObj->m_Index].ExCred += dwCost;
					ExPCPointSystem.InitPCPointForPlayer(&gObj[aSourceIndex],ExUser[aSourceIndex].PCPoint,gObj[aSourceIndex].ExCred);
					ExPCPointSystem.InitPCPointForPlayer(lpObj,ExUser[lpObj->m_Index].PCPoint,lpObj->ExCred);
				}
				else if(!strcmp(lpObj->m_szPShopText,".::WCOINT SHOP::."))
				{
					//gObj[aSourceIndex].m_wCashPoint -= dwCost;
					//lpObj->m_wCashPoint += dwCost;
					GDRequestWcoinInfoLoad (&gObj[aSourceIndex],dwCost,2);
					GDRequestWcoinInfoLoad (lpObj,dwCost,1);
				}
				else if(ExUser[lpObj->m_Index].OffTrade == OFFCHAOS_DC)
				{
					gObj[aSourceIndex].ChaosBank -= dwCost;
					lpObj->ChaosBank += dwCost;
					ExUserDataSend(aSourceIndex);
				}
				else if(ExUser[lpObj->m_Index].OffTrade == OFFBLESS_DC)
				{
					gObj[aSourceIndex].BlessBank -= dwCost;
					lpObj->BlessBank += dwCost;
					ExUserDataSend(aSourceIndex);
				}
				else if(ExUser[lpObj->m_Index].OffTrade == OFFSOUL_DC)
				{
					gObj[aSourceIndex].SoulBank -= dwCost;
					lpObj->SoulBank += dwCost;
					ExUserDataSend(aSourceIndex);
				}
				else if(ExUser[lpObj->m_Index].OffTrade == OFFLIFE_DC)
				{
					gObj[aSourceIndex].LifeBank -= dwCost;
					lpObj->LifeBank += dwCost;
					ExUserDataSend(aSourceIndex);
				}
				else
				{
				...
				GDUserItemSave(lpObj);	//OffTrade
				GJSetCharacterInfo(lpObj, lpObj->m_Index, 0);	//OffTrade
				//--------------------------------
				//#################################################################
				...
				if (PShop_CheckInventoryEmpty(lpObj->m_Index) == true )
				{
					...
					//	OffTrade
					if(ExUser[lpObj->m_Index].OffTrade != 0)
					{
						ExUser[lpObj->m_Index].OffTrade = 0;
						GJPUserClose(lpObj->AccountID);
...
void CGChaosBoxItemMixButtonClick(PMSG_CHAOSMIX* aRecv, int aIndex) 
{
	...
	if(ChaosMixManager.Main(aIndex, iMixType)) return;
#endif
	...
	switch ( iMixType )
	{
		...
		case CHAOS_TYPE_SOCKET_SPHERE_REMOVE_MIX:
			...
			break;
		case 50:
			g_MixSystem.MixWings2_5(lpObj);
			break;
		case 51:
			g_MixSystem.MixWings4rd(lpObj);
			break;
#if(_GRAND_HERO_SYSTEM_)
		case 52:
			GrandHero.MixGrandItem(lpObj);
			break;
#endif
#ifdef _WINGS_5LVL_
		case 53:
			g_MixSystem.MixWings5rd(lpObj);
			break;
#endif
			//season4 add-on end
		default:
...
void PMoveProc(PMSG_MOVE* lpMove, int aIndex)
{
	...
	if( lpObj->Type == OBJ_USER && lpObj->MapNumber == MAP_INDEX_CASTLESIEGE )
	{
		int nextX = lpObj->X;
		int nextY = lpObj->Y;
...
void CGAttack(PMSG_ATTACK* lpMsg, int aIndex) 
{
	...
	gObjAttack(lpObj, lpTargetObj, NULL, FALSE, FALSE, 0, FALSE,0);
...
void CGMagicAttack(PMSG_MAGICATTACK* lpMsg, int aIndex)
{
	...
	if ( lpObj->Type == OBJ_USER )
	{
		...
		short Energy = lpObj->Energy + lpObj->AddEnergy; //season 3.5 add-on
		short Strength = lpObj->Strength + lpObj->AddStrength; //season 3.5 add-on
		short Dexterity = lpObj->Dexterity + lpObj->AddDexterity; //season 3.5 add-on 
...
void CGTeleportRecv(PMSG_TELEPORT* lpMsg, int aIndex) 
{
		...
		if ( lpMagic )
		{
			...
			if ( usemana >= 0 )
			{
				...
				if ( usebp >= 0 )
				{
					...
					GCManaSend(gObj[aIndex].m_Index, (int)gObj[aIndex].Mana, -1, 0, (int)gObj[aIndex].BP);
				}
			}
		}
	}
	...
	if(gComboAttack.CheckCombo(aIndex, 6))
	{
		bCombo = 1;
	}

	gObj[aIndex].NewCombo = bCombo;
#endif
}
...
void CGBeattackRecv(BYTE* lpRecv, int aIndex, int magic_send) 
{
	...
	for (int n=0;n<lpCount->Count;n++)
	{
		...
		if(ExUser[aIndex].OffExp != 1) //OffExp
		{
			//season4 add-on
		...
		if ( tNumber >= 0 && tNumber < OBJMAX-1 )
		{
			if ( lpMagic->m_Skill == 55 )
			{
				if(gObj[aIndex].Type == OBJ_USER) //season 3.5 add-on
				{
					if(gObj[aIndex].Strength+gObj[aIndex].AddStrength >= 596)
					{
						...
						gObjAttack(&gObj[aIndex], &gObj[tNumber], lpMagic, TRUE, 1, 0, FALSE,0);
					}
				}
				else
				{
			...
			{
				gObjAttack(&gObj[aIndex], &gObj[tNumber], lpMagic, FALSE, 1, 0, FALSE,0);
			}
		}
...
void CGUseItemRecv(PMSG_USEITEM* lpMsg, int aIndex) //0045C690
{
		...
		gObj[aIndex].m_IfState.type != 3 )
	{
		//////////////////////
		GCReFillSend(aIndex, 
			(int)gObj[aIndex].Life, 
			0xFD, 
			TRUE, 
			(int)gObj[aIndex].iShield);
	...
	if ( citem->IsItem() )
	{
			...
			citem->m_Type == ITEMGET(14,3) )
		{
			...
			if ( gObj[aIndex].FillLife > 0.0f )
			{
				...
				GCReFillSend(gObj[aIndex].m_Index, (int)gObj[aIndex].Life, 0xFF, FALSE, (int)gObj[aIndex].iShield);
			}
		...
		else if ( citem->m_Type == ITEMGET(14,63) || citem->m_Type == ITEMGET(14,64) )
		{
			...
			case ITEMGET(14,63):
				{
					gObj[aIndex].Life = gObj[aIndex].MaxLife + gObj[aIndex].AddLife;
					GCReFillSend(aIndex,(int)gObj[aIndex].Life, -1, 0,(int)gObj[aIndex].iShield);
				}
				break;
			case ITEMGET(14,64):
				{
					gObj[aIndex].Mana = gObj[aIndex].MaxMana + gObj[aIndex].AddMana;
					GCManaSend(aIndex,(int)gObj[aIndex].Mana, -1, 0, (int)gObj[aIndex].BP);
				}
				break;
			}
		...
		else if ( gNewJewels.CheckJewel(citem->m_Type) ) // Jewel Of Exellent
		{
			gNewJewels.JewelMain(&gObj[aIndex],lpMsg->inventoryPos, lpMsg->invenrotyTarget);
		}
#else
		//########################################################################################
		//								Start New Jewels
		//########################################################################################
		else if ( citem->m_Type == ITEMGET(14,180) ) // Jewel Of Exellent
		{
			if ( ExJewelOfExellentAdded(&gObj[aIndex], lpMsg->inventoryPos, lpMsg->invenrotyTarget) == TRUE )
			{
				gObjInventoryItemSet(aIndex, pos, -1);
				gObj[aIndex].pInventory[pos].Clear();
				GCInventoryItemOneSend(aIndex, lpMsg->invenrotyTarget);
		...
		//---------------------------------------------------------------------------------------
		else if ( citem->m_Type == ITEMGET(14,181) ) // Jewel Of Divinity
		{
			if ( ExJewelOfDivinityAdded(&gObj[aIndex], lpMsg->inventoryPos, lpMsg->invenrotyTarget) == TRUE )
			{
				gObjInventoryItemSet(aIndex, pos, -1);
				gObj[aIndex].pInventory[pos].Clear();
				GCInventoryItemOneSend(aIndex, lpMsg->invenrotyTarget);
		...
		//---------------------------------------------------------------------------------------
		else if ( citem->m_Type == ITEMGET(14,182) ) // Jewel Of Luck
		{
			if ( ExJewelOfLuckAdded(&gObj[aIndex], lpMsg->inventoryPos, lpMsg->invenrotyTarget) == TRUE )
			{
				gObjInventoryItemSet(aIndex, pos, -1);
				gObj[aIndex].pInventory[pos].Clear();
				GCInventoryItemOneSend(aIndex, lpMsg->invenrotyTarget);
		...
		else if ( citem->m_Type == ITEMGET(13,48) )
		{
				...
				gObj[aIndex].MapNumber != MAP_INDEX_KALIMA7 )
			{
				...
				{
					GCItemDurSend(aIndex,(BYTE)pos,(BYTE)gObj[aIndex].pInventory[pos].m_Durability,1); //GS 19 Second Edition
...
void CGRelationShipReqJoinBreakOff(PMSG_RELATIONSHIP_JOIN_BREAKOFF_REQ * aRecv, int aIndex) 
{
	...
	if ( aRecv->btRequestType == 1 )
	{
		...
		{
			//int AllianceMinPlayers = GetPrivateProfileInt("Guild","AllianceMinPlayers",20,"..\\ExTeam\\CommonServer.ini");
			if ( ExConfig.CommonServer.AllianceMinPlayers > lpTargetGuildInfo->Count )
			{
...
void GCAnsRegGuildMark(int iIndex, int iResult, CSP_ANS_GUILDREGMARK* lpMsgResult) 
{
	...
	else
	{
		...
		else
		{
			...
			{
				::GCItemDurSend(iIndex, (BYTE)lpMsgResult->iItemPos, (BYTE)gObj[iIndex].pInventory[lpMsgResult->iItemPos].m_Durability, 0);
			}
			else
			{
...
void CGReqLuckyCoinRegister(int iIndex, BYTE Result, int Coin, BYTE Ipos) //identical
{
	...
	if(Result == 1)
	{
		...
		{
			GCItemDurSend2(iIndex, Ipos, (BYTE)gObj[iIndex].pInventory[Ipos].m_Durability, 0);
		}
...
void CGReqLuckyCoinTrade(PMSG_REQ_TRADE_LUCKYCOIN* lpMsg, int iIndex) //identical
{
	...
	for(n = INVETORY_WEAR_SIZE; n < MAIN_INVENTORY_SIZE; n++) //5
	{
		if( gObj[iIndex].pInventory[n].IsItem() == TRUE &&
			gObj[iIndex].pInventory[n].m_Type == ITEMGET(14,100))
		{
			LuckyCoinCount = (int)(LuckyCoinCount + (float)gObj[iIndex].pInventory[n].m_Durability);
		}
	}
	...
	else
	{
		...
		for(n = INVETORY_WEAR_SIZE; n < MAIN_INVENTORY_SIZE; n++) //5
		{
			if( gObj[iIndex].pInventory[n].IsItem() == TRUE && gObj[iIndex].pInventory[n].m_Type == ITEMGET(14,100))
			{
				...
				{
					LuckyCoinNumber -= (int)gObj[iIndex].pInventory[n].m_Durability;
				...
				else
				{
					...
					GCItemDurSend2(lpObj->m_Index, (BYTE)n, (BYTE)gObj[iIndex].pInventory[n].m_Durability, 0);
					break; //
				}
			}
		}
