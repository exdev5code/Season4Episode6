...
void CIllusionTempleProcess::ProcState_Closed(int iTick) 
{
	...
	if(this->m_iLeftMinutesBeforeOpen <= 0)
	{
		if(this->m_iEventBattleTimer <= 0)
		{
			...
			//int Ex_IllusionTemplPlaerMin = GetPrivateProfileInt("IllusionTemple","ItMinPlayers",4,"..\\ExTeam\\Events.ini");

			if(this->m_iEnteredUserCount < ExConfig.Event.Ex_IllusionTemplPlaerMin) //UserCount
			{
						...
						if( gObj[this->m_FloorData[n].m_iTeamPlayerIndex].MapNumber == (this->m_btFloorIndex + MAP_INDEX_ILLUSIONTEMPLE) )
						{
							...
							if(loc142 > -1)
							{
								if(gObj[this->m_FloorData[n].m_iTeamPlayerIndex].pInventory[loc142].m_Type == ITEMGET(13,51))
								{
									...
									GCItemDurSend(gObj[this->m_FloorData[n].m_iTeamPlayerIndex].m_Index, (BYTE)loc142,  (BYTE)gObj[this->m_FloorData[n].m_iTeamPlayerIndex].pInventory[loc142].m_Durability, 0);
								}
		...
		if(this->m_iEventBattleTimer <= 1000 * (m_i_IT_RestTime * 60) - 3000)
		{
			if(this->m_iEventBattleTimer > 0)
			{
				if(this->m_bIllusionTempleEventEnded == FALSE)
				{
					...
					if(this->m_iBallPlayerIndex >= 0 && FindItemBallPosition(iTick) >= 0)
					{
						if(gObj[this->m_iBallPlayerIndex].pInventory[FindItemBallPosition(iTick)].m_Type == ITEMGET(14,64))
						{
							::gObjInventoryDeleteItem(this->m_iBallPlayerIndex, FindItemBallPosition(iTick));
							::GCInventoryItemDeleteSend(this->m_iBallPlayerIndex, FindItemBallPosition(iTick), 0);
...
void CIllusionTempleProcess::NpcStatueProc(LPOBJ lpObj, LPOBJ lpTargetObj) 
{
	...
	ItemSerialCreateSend(lpTargetObj->m_Index, (BYTE)lpTargetObj->MapNumber, (BYTE)lpObj->X, (BYTE)lpObj->Y, loc3, (BYTE)loc4, (BYTE)loc2, (BYTE)loc5, (BYTE)loc6, (BYTE)loc7, lpTargetObj->m_Index, (BYTE)loc10, 0);
...
void CIllusionTempleProcess::NpcAlliedPedestalProc(LPOBJ lpObj, LPOBJ lpTargetObj) 
{
	...
	if(lpTargetObj->pInventory[FindItemBallPosition(lpTargetObj->m_Index)].m_Type == ITEMGET(14,64))
	{
...
{
	if(lpObj->pInventory[FindItemBallPosition(lpObj->m_Index)].m_Type == ITEMGET(14,64))
	{

		int ItemType = ITEMGET(14,64);
		...
		//gObjInventoryDeleteItem(lpObj->m_Index, iItemPos);
		//::gObjInventoryDeleteItem(lpObj->m_Index, iItemPos);
		gObjDeleteItemsCount(lpObj->m_Index,ItemType,0,1);

		GCInventoryItemDeleteSend(lpObj->m_Index, FindItemBallPosition(lpObj->m_Index), 0);
		LogAddTD("[Illusion Temple] (%d) (%s)(%s) to Delete Relics Item [Serial:%u]", this->m_btFloorIndex+1, lpObj->AccountID, lpObj->Name, lpObj->pInventory[this->m_btBallInventoryPosition].m_Number);
	}
}
...
void CIllusionTempleProcess::DropRelicsItem(int iIndex) 
{
	...
	if(FindItemBallPosition(lpObj->m_Index) == -1)
	{
		return;
	}
	...
	int iItemPos = FindItemBallPosition(lpObj->m_Index); //item position loc3
	...
	{
		LogAddTD("[Illusion Temple] (%d) (%s)(%s) to Drop Relics Item [Serial:%u]",this->m_btFloorIndex+1, lpObj->AccountID, lpObj->Name, lpObj->pInventory[FindItemBallPosition(lpObj->m_Index)].m_Number);
...
void CIllusionTempleProcess::UnlockTeamPedestal(int aIndex, BYTE pos)
{
	...
	this->m_btBallInventoryPosition = FindItemBallPosition(aIndex);
...
void CIllusionTempleProcess::CalcSendRewardEXP() 
{
	...
	for(i = 0;i < MAX_FLOOR_USER;i++)
	{
		...
		if(this->m_FloorData[i].m_iTeamPlayerIndex != -1 && gObj[this->m_FloorData[i].m_iTeamPlayerIndex].Connected > PLAYER_LOGGED && gObj[this->m_FloorData[i].m_iTeamPlayerIndex].MapNumber == this->m_btFloorIndex+MAP_INDEX_ILLUSIONTEMPLE &&	this->m_FloorData[i].m_btUsedKillCountPoint >= g_iIllusionTempleMinUseSkillCnt)
		{
			...
			gObjPandaUserSetExp(&gObj[this->m_FloorData[i].m_iTeamPlayerIndex],this->m_FloorData[i].m_i64ExpReward);	//Panda Use
			gObjUserExperienceMain(&gObj[this->m_FloorData[i].m_iTeamPlayerIndex],this->m_FloorData[i].m_i64ExpReward);
...
void CIllusionTempleProcess::DropRewardItem(int aIndex)
{
	...
	int loc2 = aIndex;
	//Configs
	//int Ex_ItDropType	= GetPrivateProfileInt("IllusionTemple","ItDropType",12,"..\\ExTeam\\Events.ini");
	//int Ex_ItDropID		= GetPrivateProfileInt("IllusionTemple","ItDropID",15,"..\\ExTeam\\Events.ini");
	//int Ex_ItDropLevel	= GetPrivateProfileInt("IllusionTemple","ItDropLevel",0,"..\\ExTeam\\Events.ini");
	//int Ex_ItDropSkill	= GetPrivateProfileInt("IllusionTemple","ItDropSkill",0,"..\\ExTeam\\Events.ini");
	//int Ex_ItDropLuck	= GetPrivateProfileInt("IllusionTemple","ItDropLuck",0,"..\\ExTeam\\Events.ini");
	//int Ex_ItDropOpt	= GetPrivateProfileInt("IllusionTemple","ItDropOpt",0,"..\\ExTeam\\Events.ini");
	//int Ex_ItDropExcOpt	= GetPrivateProfileInt("IllusionTemple","ItDropExcOpt",0,"..\\ExTeam\\Events.ini");
	//int Ex_ItDropAncOpt	= GetPrivateProfileInt("IllusionTemple","ItDropAncOpt",0,"..\\ExTeam\\Events.ini");

	int Exe1 = 0;
	int Anc1 = 0;

	if ( ExConfig.Event.Ex_ItDropExcOpt > 0 )
		Exe1 = GenExcOpt(ExConfig.Event.Ex_ItDropExcOpt);

	if ( ExConfig.Event.Ex_ItDropAncOpt == 5 || ExConfig.Event.Ex_ItDropAncOpt == 10 )
		Anc1 = ExConfig.Event.Ex_ItDropAncOpt;

	int loc3 = ItemGetNumberMake(ExConfig.Event.Ex_ItDropType, ExConfig.Event.Ex_ItDropID);	// Jewel of Chaos
	ItemSerialCreateSend(gObj[aIndex].m_Index, (BYTE)gObj[aIndex].MapNumber, (BYTE)gObj[aIndex].X, gObj[aIndex].Y,loc3,
		(BYTE)ExConfig.Event.Ex_ItDropLevel, 0, (BYTE)ExConfig.Event.Ex_ItDropSkill, (BYTE)ExConfig.Event.Ex_ItDropLuck, 
		(BYTE)ExConfig.Event.Ex_ItDropOpt, loc2, (BYTE)Exe1, (BYTE)Anc1);
}
