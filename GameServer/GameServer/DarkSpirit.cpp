...
BOOL CDarkSpirit::Attack(LPOBJ lpObj, LPOBJ lpTargetObj, CMagicInf * lpMagic, int criticaldamage, int iActionType)
{
	...
	if ( gObjWingSprite(lpTargetObj) == TRUE )
	{
		...
		if ( AttackDamage > 1 )
		{
			...
			else if ( Wing->m_Type >= ITEMGET(12, 186) || Wing->m_Type <= ITEMGET(12, 188) )	//Custom Wings2.5
			{
		...
		if ( g_CastleSiege.GetCastleState() == CASTLESIEGE_STATE_STARTSIEGE)
		{
			if ( lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER )
			{
				if ( lpObj->MapNumber == MAP_INDEX_CASTLESIEGE && lpTargetObj->MapNumber == MAP_INDEX_CASTLESIEGE )
				{
					if( lpObj->m_btCsJoinSide == lpTargetObj->m_btCsJoinSide)
					{
						//int PetAttackByNPC = GetPrivateProfileInt("CastleSiegePet","PetAttackByNPC",20,"..\\ExTeam\\Skills.ini");
						AttackDamage = ( AttackDamage * ExConfig.Skills.PetAttackByNPC ) / 100;		//AttackDamage = AttackDamage * 20 / 100;
					}
					else if(g_ShieldSystemOn == FALSE)
					{
						//int PetAttackByPlayer = GetPrivateProfileInt("CastleSiegePet","PetAttackByPlayer",40,"..\\ExTeam\\Skills.ini");
						AttackDamage = AttackDamage * ExConfig.Skills.PetAttackByPlayer / 100;	//AttackDamage = AttackDamage * 40 / 100;
					}
				}
			}
		}
...
void __cdecl CDarkSpirit::SendLevelmsg(int aIndex, int nPos, int PetType, int InvenType)
{
	...
	pMsg.Life = (BYTE)gObj[aIndex].pInventory[nPos].m_Durability; //season4 add-on (possible fix)
