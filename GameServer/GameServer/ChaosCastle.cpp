	...
	while ( true )
	{
		...
		while ( true )
		{
			...
			else if ( type == 4 )
			{
				...
				int iChaosCastleIndex = (int)TokenNumber;
...
void CChaosCastle::SearchNDropMonsterItem(int iChaosCastleIndex, int iMonsterIndex, int iMaxHitUserIndex)
{
	...
	::ItemSerialCreateSend(gObj[iMaxHitUserIndex].m_Index, (BYTE)gObj[iMaxHitUserIndex].MapNumber, (BYTE)gObj[iMaxHitUserIndex].X,
		(BYTE)gObj[iMaxHitUserIndex].Y, iType, (BYTE)iLevel, 0, 0, 0, 0, iMaxHitUserIndex, 0, 0);
	...
	if ( gObj[iUserIndex].Type == OBJ_USER )
	{
		while ( iCAL_EXP > 0 )
		{
			if ( iCAL_EXP > 0 )
			{
				...
				gObjPandaUserSetExp(&gObj[iUserIndex],iCAL_EXP);	//Panda Use
				gObjUserExperienceMain(&gObj[iUserIndex],iCAL_EXP);

				iCAL_EXP = (__int64)((int)(iCAL_EXP) * this->m_stChaosCastleData[gObj[iUserIndex].m_cChaosCastleIndex].m_iCC_REWARD_EXP); //season 4.5 add-on
...
bool CChaosCastle::CheckCanStartPlay(int iChaosCastleIndex)
{
	...
	if ( iEnteredUser < ExConfig.Event.Ex_CCMinPlayer )
	{
...
GOTO_ItemCreate: //season4.5 add-on
	ItemSerialCreateSend(gObj[iWinnerIndex].m_Index, (BYTE)gObj[iWinnerIndex].MapNumber, (BYTE)gObj[iWinnerIndex].X, (BYTE)gObj[iWinnerIndex].Y, iItemType, (BYTE)iItemLevel, (BYTE)iItemDur, 0, 0, 0, iWinnerIndex, 0, 0);
}
