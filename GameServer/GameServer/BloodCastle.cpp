...
bool CBloodCastle::BloodCastleChaosMix(int iIndex, int iLEVEL) //0054B2A0
{
	...
	if ( (rand()%100) < iMIX_SUCCESS_RATE )
	{
		...
		ItemSerialCreateSend(iIndex, -1, 0, 0, item_num, (BYTE)iLEVEL, 255, 0, 0, 0, -1, 0, 0);
	...
	if ( gObj[iIndex].Type == OBJ_USER )
	{
		while ( iCAL_EXP > 0 )
		{
			if ( iCAL_EXP > 0 )
			{
				...
				gObjPandaUserSetExp(&gObj[iIndex],iCAL_EXP);	//Panda Use
				gObjUserExperienceMain(&gObj[iIndex],iCAL_EXP);

				iCAL_EXP = (__int64)((int)(iCAL_EXP) * this->m_BridgeData[gObj[iIndex].m_cBloodCastleIndex].m_iBC_REWARD_EXP); //season 4.5 add-on
...
void CBloodCastle::DropChaosGem(int iIndex) //00552850
{
	...
	////Configs
	//int Ex_BCDropType	= GetPrivateProfileInt("BloodCastle","BCDropType",12,"..\\ExTeam\\Events.ini");
	//int Ex_BCDropID		= GetPrivateProfileInt("BloodCastle","BCDropID",15,"..\\ExTeam\\Events.ini");
	//int Ex_BCDropLevel	= GetPrivateProfileInt("BloodCastle","BCDropLevel",0,"..\\ExTeam\\Events.ini");
	//int Ex_BCDropSkill	= GetPrivateProfileInt("BloodCastle","BCDropSkill",0,"..\\ExTeam\\Events.ini");
	//int Ex_BCDropLuck	= GetPrivateProfileInt("BloodCastle","BCDropLuck",0,"..\\ExTeam\\Events.ini");
	//int Ex_BCDropOpt	= GetPrivateProfileInt("BloodCastle","BCDropOpt",0,"..\\ExTeam\\Events.ini");
	//int Ex_BCDropExcOpt	= GetPrivateProfileInt("BloodCastle","BCDropExcOpt",0,"..\\ExTeam\\Events.ini");
	//int Ex_BCDropAncOpt	= GetPrivateProfileInt("BloodCastle","BCDropAncOpt",0,"..\\ExTeam\\Events.ini");

	int Exe1 = 0;
	int Anc1 = 0;

	if ( ExConfig.Event.Ex_BCDropExcOpt > 0 )
		Exe1 = GenExcOpt(ExConfig.Event.Ex_BCDropExcOpt);

	if ( ExConfig.Event.Ex_BCDropAncOpt == 5 || ExConfig.Event.Ex_BCDropAncOpt == 10 )
		Anc1 = ExConfig.Event.Ex_BCDropAncOpt;

	int iType = ItemGetNumberMake(ExConfig.Event.Ex_BCDropType, ExConfig.Event.Ex_BCDropID);

	ItemSerialCreateSend(gObj[iIndex].m_Index, (BYTE)iMapNumber, (BYTE)gObj[iIndex].X, (BYTE)gObj[iIndex].Y, iType, 
		(BYTE)ExConfig.Event.Ex_BCDropLevel, 0, (BYTE)ExConfig.Event.Ex_BCDropSkill, (BYTE)ExConfig.Event.Ex_BCDropLuck, 
		(BYTE)ExConfig.Event.Ex_BCDropOpt, iMaxHitUser, (BYTE)Exe1, (BYTE)Anc1);
}
