	...
	while ( true )
	{
		...
		else
		{
			if ( Token == 1 )
			{
				...
				Token=GetToken();
				this->m_TargetGate[number]=(WORD)TokenNumber;

				Token=GetToken();
				this->m_Dir[number]=(BYTE)TokenNumber;

				Token=GetToken();
				this->m_Level[number]=(short)TokenNumber;
			}
		}
	}
...
BOOL CGate::IsInGate(int aIndex, int GateNumber)
{
	...
	LogAddC(2,"lpObj->MapNumber: %d this->m_MapNumber[GateNumber] :%d",lpObj->MapNumber,this->m_MapNumber[GateNumber]);
