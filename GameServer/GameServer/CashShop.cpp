...
#if CASH_SHOP_ADDITIONAL == 1
int g_iCategoryAnc = -1;
int g_iCategoryExlAnc = -1;
int g_iCategoryBuff = -1;
#endif
	...
	while ( true )
	{
		...
		while ( true )
		{
			...
			if ( iType == 1 )
			{
				...
				ItemStatus.btItemType = (BYTE)TokenNumber;

				Token = (SMDToken)GetToken();
				ItemStatus.wItemIndex = (WORD)TokenNumber;

				Token = (SMDToken)GetToken();
				ItemStatus.btItemLevel = (BYTE)TokenNumber;


				Token = (SMDToken)GetToken();
				ItemStatus.btSkillOption = (BYTE)TokenNumber;

				Token = (SMDToken)GetToken();
				ItemStatus.btLuckOption = (BYTE)TokenNumber;

				Token = (SMDToken)GetToken();
				ItemStatus.btAddOption = (BYTE)TokenNumber;

				BYTE btExOptionValue = 0;
				BYTE btExOption = 0;

				Token = (SMDToken)GetToken();
				btItemExOption = (BYTE)TokenNumber;

				if ( btItemExOption > 0 && btItemExOption < 64)
				{
					//btExOptionValue = 1 << (int)(btItemExOption-1);
					//btExOption |= btExOptionValue;
					//ItemStatus.btExOption = btExOption;
					ItemStatus.btExOption = btItemExOption;
				}
				else
				{
				...
				//add
				Token = (SMDToken)GetToken();
				btItemAncOption = (BYTE)TokenNumber;

				if(btItemAncOption == 5 || btItemAncOption == 10)
				{
					ItemStatus.btAncOption = btItemAncOption;
				}
				else
				{
					ItemStatus.btAncOption = 0;
				}
				//---
				Token = (SMDToken)GetToken();
				btItemX = (BYTE)TokenNumber;

				Token = (SMDToken)GetToken();
				btItemY = (BYTE)TokenNumber;

				Token = (SMDToken)GetToken();
				btItemScale = (BYTE)TokenNumber;
				...
				if(g_SocketOption.CheckItemType(iItemCode) == 1) 
				{
					BYTE CA_Item_socket[5] = {0xFE,0xFE,0xFE,0xFE,0xFE};
					ItemByteConvert(ItemStatus.btItemInfo, iItemCode, ItemStatus.btSkillOption,
						ItemStatus.btLuckOption, ItemStatus.btAddOption,ItemStatus.btItemLevel, 1, ItemStatus.btExOption, ItemStatus.btAncOption, 0, 0, CA_Item_socket, 0xFF);
				}
				else
				{
					ItemByteConvert(ItemStatus.btItemInfo, iItemCode, ItemStatus.btSkillOption,
						ItemStatus.btLuckOption, ItemStatus.btAddOption,ItemStatus.btItemLevel, 1, ItemStatus.btExOption, ItemStatus.btAncOption, 0, 0, NULL, 0xFF);
				}
#else
				ItemByteConvert(ItemStatus.btItemInfo, iItemCode, ItemStatus.btSkillOption,
					ItemStatus.btLuckOption, ItemStatus.btAddOption,ItemStatus.btItemLevel, 1, ItemStatus.btExOption, ItemStatus.btAncOption, 0, 0, NULL, 0xFF);	//add
...
#if CASH_SHOP_ADDITIONAL == 1
	g_iCategoryBuff = GetPrivateProfileInt("CashShopOption", "CategoryBuff", 0, pchFilename);
	g_iCategoryAnc = GetPrivateProfileInt("CashShopOption", "CategoryAnc", -1, pchFilename);
	g_iCategoryExlAnc = GetPrivateProfileInt("CashShopOption", "CategoryExlAnc", -1, pchFilename);
#endif
}
	...
	while ( true )
	{
		...
		while ( true )
		{
			...
			if ( iType == 1 )
			{
				...
				Token = (SMDToken)GetToken();
				btItemType = (BYTE)TokenNumber;

				Token = (SMDToken)GetToken();
				wItemIndex = (WORD)TokenNumber;

				Token = (SMDToken)GetToken();
				btSaleRatio = (BYTE)TokenNumber;

				Token = (SMDToken)GetToken();
				btDuration = (BYTE)TokenNumber;

				Token = (SMDToken)GetToken();
				wPrice = (WORD)TokenNumber;
...
#if CASH_SHOP_ADDITIONAL == 1
		if(iCategory == g_iCategoryBuff)
		{
			this->CashItemList[iCategory][iItemIndex+iIndex].btItemExOption = 0;
			this->CashItemList[iCategory][iItemIndex+iIndex].btItemAncOption = 0;
		}
		else if(iCategory == g_iCategoryExlAnc)
		{
			this->CashItemList[iCategory][iItemIndex+iIndex].btItemExOption = 63;
			if(ITEMGET(this->CashItemList[iCategory][iItemIndex+iIndex].btItemType,this->CashItemList[iCategory][iItemIndex+iIndex].wItemIndex)==ITEMGET(13,9) ||
				ITEMGET(this->CashItemList[iCategory][iItemIndex+iIndex].btItemType,this->CashItemList[iCategory][iItemIndex+iIndex].wItemIndex)==ITEMGET(13,24))
			{
				this->CashItemList[iCategory][iItemIndex+iIndex].btItemAncOption = 10;
			}
			else
			{
				this->CashItemList[iCategory][iItemIndex+iIndex].btItemAncOption = 5;
			}
		}
		else if(iCategory == g_iCategoryAnc)
		{
			this->CashItemList[iCategory][iItemIndex+iIndex].btItemExOption = 0;
			if(ITEMGET(this->CashItemList[iCategory][iItemIndex+iIndex].btItemType,this->CashItemList[iCategory][iItemIndex+iIndex].wItemIndex)==ITEMGET(13,9) ||
				ITEMGET(this->CashItemList[iCategory][iItemIndex+iIndex].btItemType,this->CashItemList[iCategory][iItemIndex+iIndex].wItemIndex)==ITEMGET(13,24))
...
#else
		this->CashItemList[iCategory][iItemIndex+iIndex].btItemAncOption = lpItemStatus->btAncOption;
#endif
...
void CCashShop::MakeItemListProtocol()
{
	...
	for ( iCategory=0;iCategory < MAX_CASH_SHOP_CATEGORY;iCategory++)
	{
		...
		for ( int i=0;i<iAddItemCount;i++)
		{
			...
			if(iCategory == g_iCategoryBuff)
			{
				this->CashItemListCompress[iCategory][iAddItemListIndex].btItemInfo[3] = 0;
				this->CashItemListCompress[iCategory][iAddItemListIndex].btItemInfo[4] = 0;
			}
			else if(iCategory == g_iCategoryExlAnc)
			{
				this->CashItemListCompress[iCategory][iAddItemListIndex].btItemInfo[3] = 63;	//63
				
				if(this->CashItemListCompress[iCategory][iAddItemListIndex].btItemInfo[0] == 9 || this->CashItemListCompress[iCategory][iAddItemListIndex].btItemInfo[0] == 24)
				{
					this->CashItemListCompress[iCategory][iAddItemListIndex].btItemInfo[4] = 10;
				}
				else
				{
					this->CashItemListCompress[iCategory][iAddItemListIndex].btItemInfo[4] = 5;
				}
			...
			//this->CashItemListCompress[iCategory][iAddItemListIndex].

				//this->CashItemListCompress[iCategory][iAddItemListIndex].

			//LogAddC(2,"CS ITEM: %d",this->CashItemListCompress[iCategory][iAddItemListIndex].btItemInfo[0]);

			//if(g_SocketOption.CheckItemType(this->CashItemListCompress[iCategory][iAddItemListIndex].btItemInfo[0]) == 1)
			//{
			//	this->CashItemListCompress[iCategory][iAddItemListIndex].btItemInfo[7] = 0xFE;
			//	this->CashItemListCompress[iCategory][iAddItemListIndex].btItemInfo[8] = 0xFE;
			//	this->CashItemListCompress[iCategory][iAddItemListIndex].btItemInfo[9] = 0xFE;
			//	this->CashItemListCompress[iCategory][iAddItemListIndex].btItemInfo[10] = 0xFE;
			//	this->CashItemListCompress[iCategory][iAddItemListIndex].btItemInfo[11] = 0xFE;
			//}

			//if(iCategory == g_iCategoryAnc)
			//	this->CashItemListCompress[iCategory][iAddItemListIndex].btItemInfo[4] = 5;
			//else 
			//	this->CashItemListCompress[iCategory][iAddItemListIndex].btItemInfo[4] = 0;
			//this->CashItemProtocol[iCategory][iPageIndex].ItemInfo[iItemIndex+i].btItemInfo[4] = 0;
...
BOOL CCashShop::GiveBuyItemToInventory(LPOBJ lpObj, int iItemGuid)
{
	...
	BYTE XxX[5] = {0xFE,0xFE,0xFE,0xFE,0xFE};
	...
	pItem.Convert(iItemCode, lpItemInfo->btItemSkillOpion, lpItemInfo->btItemLuckOption, lpItemInfo->btItemAddOption,
		lpItemInfo->btItemExOption, lpItemInfo->btItemAncOption, 0,NULL, 0xFF, CURRENT_DB_VERSION);

	pItem.m_Level = lpItemInfo->btItemLevel;
	pItem.m_Durability = lpItemInfo->btItemDuration;

	ItemIsBufExOption(ExOption, &pItem);
	ItemSerialCreateSend(lpObj->m_Index, 0xEC, (BYTE)lpObj->X, (BYTE)lpObj->Y, pItem.m_Type, (BYTE)pItem.m_Level,
		(BYTE)pItem.m_Durability, (BYTE)pItem.m_Option1,(BYTE) pItem.m_Option2, (BYTE)pItem.m_Option3, lpObj->m_Index,
		(BYTE)pItem.m_NewOption, (BYTE)pItem.m_SetOption);
