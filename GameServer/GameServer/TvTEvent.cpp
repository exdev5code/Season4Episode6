...
void cTvTEvent::Load()
{
	...
	this->NeedLevel = GetPrivateProfileInt("Common","NeedLevel",0,TVT_DIR);
	this->NeedReset = GetPrivateProfileInt("Common","NeedReset",0,TVT_DIR);
	this->NeedGrandReset = GetPrivateProfileInt("Common","NeedGrandReset",0,TVT_DIR);
	...
	this->RewardItemCount = GetPrivateProfileInt("Common","RewardItemCount",1,TVT_DIR);
...
void cTvTEvent::NPCClick(LPOBJ lpNpc, LPOBJ lpObj)
{
	...
	if(lpNpc->Class == this->NPC)
	{
		...
		{
			MsgNormal(lpObj->m_Index,"[TvT Event] Need Level: %d",this->NeedLevel);
			return;
		}

		if(this->NeedReset > ExUser[lpObj->m_Index].Reset)
		{
			MsgNormal(lpObj->m_Index,"[TvT Event] Need Reset: %d",this->NeedReset);
			return;
		}

		if(this->NeedGrandReset > ExUser[lpObj->m_Index].GReset)
		{
			MsgNormal(lpObj->m_Index,"[TvT Event] Need NeedGrandReset: %d",this->NeedGrandReset);
			return;
		}
...
void cTvTEvent::Reward(int aIndex)
{
	...
	for (int a=0;a<this->RewardItemCount;a++)
	{
