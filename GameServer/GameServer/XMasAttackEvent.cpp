...
BOOL CXMasMonsterHerd::MonsterHerdItemDrop(LPOBJ lpObj) //004B3660 identical
{
	...
	if ( lpObj->Class == 466 )
	{
		if ( (rand()%100) < g_iXMasAttackEventDropRate )
		{
			...
			ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, (BYTE)lpObj->X, (BYTE)lpObj->Y, itemnumber, 0, 10, 0, 0, 0, iIndex, 0, 0);
			return TRUE;
		}
	...
	while ( true )
	{
		...
		while ( true )
		{
			...
			else if ( type == 2 )
			{
				...
				pXMD.m_iType = (int)TokenNumber;

				Token = GetToken();
				pXMD.m_bDoRegen = (bool)TokenNumber;

				Token= GetToken();
				pXMD.m_bDoAttackFirst = (bool)TokenNumber;
			...
			else if ( type == 3 )
			{
				...
				pXET.m_iHour = (int)TokenNumber;

				Token = GetToken();
				pXET.m_iMinute = (int)TokenNumber;
			...
			else if ( type == 4 )
			{
				...
				pXMP.m_btMapNumber = (BYTE)TokenNumber;

				Token = GetToken();
				pXMP.m_iMinX = (int)TokenNumber;

				Token = GetToken();
				pXMP.m_iMinY = (int)TokenNumber;

				Token = GetToken();
				pXMP.m_iMaxX = (int)TokenNumber;

				Token = GetToken();
				pXMP.m_iMaxY = (int)TokenNumber;
