...
public:
	...
	char* GetDataBuffer();
	DWORD SetInfo(int iCountryCode, BYTE cPartition, short iServerCode, char* szServerVersion,  char* szServerName, int ServerType,  void  (*callbackfunc)(int, int));
...
private:
  ...
  int m_iTimer;
  DWORD m_bDllHandle;
  DWORD  (*WZAGSerInfo)(int, BYTE, short, char*, char*, int, void  (*)(int, int));
  void  (*WZAGGetClientVersion)(DWORD, char*, char*);
  void  (*WZAGSetSubInfo)(DWORD, int, int, int, int, float, int);
  void  (*WZAGGetKey)(DWORD, char*, int, int);
  void  (*WzAGRequestData)(DWORD, int);
  int  (*WzAGGetDataBufferSize)(DWORD);
  char*  (*WzAGGetDataBuffer)(DWORD);
  int  (*WZAGConnectSetFS)(DWORD);
  void  (*WzAGReleaseHandle)(DWORD);
