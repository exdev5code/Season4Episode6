				...
				for( int j = 0; j < 15; j++ )
				{
					if( this->SubItemList[i][j].ItemIndex != NULL )
					{
						...
						if( this->SubItemList[i][j].SetIndex != -1 )
						{
							...
							ItemSerialCreateSend(aIndex,gObj[aIndex].MapNumber,(BYTE)gObj[aIndex].X,(BYTE)gObj[aIndex].Y,
								this->SubItemList[i][j].ItemIndex,(BYTE)iLevel,(BYTE)iDur,(BYTE)iSkill,(BYTE)iLuck,(BYTE)iOption,-1,(BYTE)btNewOption,(BYTE)iSetOption);
						}
						else
						{
							iSkill = this->SubItemList[i][j].Skill;
							...
							ItemSerialCreateSend(aIndex,gObj[aIndex].MapNumber,(BYTE)gObj[aIndex].X,(BYTE)gObj[aIndex].Y,
								this->SubItemList[i][j].ItemIndex,(BYTE)iLevel,(BYTE)iDur,(BYTE)iSkill,(BYTE)iLuck,(BYTE)iOption,-1,btNewOption,iSetOption);
						}
					}
				}
			}
