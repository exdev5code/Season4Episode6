	...
	while ( true )
	{
		...
		if ( Token == NUMBER )
		{
			...
			while ( true )
			{
				...
				Token = (SMDToken)GetToken();
				p->iRandomWeight = (BYTE)TokenNumber;

				Token = (SMDToken)GetToken();
				p->iRequireLevel = (BYTE)TokenNumber;

				for ( int i=0;i<MAX_JOH_ITEM_OPTION;i++)
				{
					Token = (SMDToken)GetToken();
					p->iItemEffectValue[i] = (BYTE)TokenNumber;

					Token = (SMDToken)GetToken();
					p->iZenForRestore[i] = (DWORD)TokenNumber;
				}
...
BOOL CJewelOfHarmonySystem::StrengthenItemByJewelOfHarmony(LPOBJ lpObj, int source, int target)
{
	...
	//int ExAddJoHtoAnc = GetPrivateProfileInt("Items","AddJoHtoAnc",1,"..\\ExTeam\\CommonServer.ini");
	if(!ExConfig.CommonServer.ExAddJoHtoAnc)
	{
