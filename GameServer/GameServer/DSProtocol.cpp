		...
		for ( int n=0;n<pCLCount.Count ;n++)
		{
			...
			else	// Case DB 3
			{
				...
				if ( (TempInventory[8] & 0x04) == 0 )
				{
					...
					else if(TempInventory[8] != 0x43)//Season 4.5 Addon
					{
						index |= (TempInventory[8] ) & 0x03;
					}
				}
				...
				//Custom Wings4
				if ( TempInventory[7] >= 180 && TempInventory[7] <= 188 )
				{
					pCList.CharSet[5] |= CS_SET_WING1(3);
					pCList.CharSet[17] |= (BYTE)(TempInventory[7] - 180 + 1) << 2;
				}
#ifdef _WINGS_5LVL_
				if ( TempInventory[7] >= 189 && TempInventory[7] <= 194 )
				{
					pCList.CharSet[5] |= CS_SET_WING1(3);
					pCList.CharSet[17] |= (BYTE)(TempInventory[7] - 180 + 1) << 2;
				}
#endif
				if ( (TempInventory[8] & 0x04) == 0 && TempInventory[8] != 0x1FF ) //Season 3.5 CashShop Pet Display
				{
					BYTE btNewValue = 0;
					...
					case 65:
						btNewValue = 0x40;
						break;
					case 67:// Season 4.5 addon
						btNewValue = 0x80;
						break;
					case 80:// Season 4.6 addon
						btNewValue |= 224;
						break;
					}
...
void GJSetCharacterInfo(LPOBJ lpObj, int aIndex, BOOL bMapServerMove)
{
	...
	pCSave.PCPoint = ExUser[aIndex].PCPoint;
	pCSave.Reset = ExUser[aIndex].Reset;
	pCSave.GReset = ExUser[aIndex].GReset;
	pCSave.ExFreePoints = ExUser[aIndex].ExFreePoints;	//ExAdd
	pCSave.ExQuestNum = ExUser[aIndex].ExQuestNum;	//ExAdd
	pCSave.ExQuestKill = ExUser[aIndex].ExQuestKill;	//ExAdd
	pCSave.ExCred = lpObj->ExCred;
//#ifdef PREMIUM_SYSTEM
	pCSave.PremiumTime = ExUser[aIndex].PremiumTime;
//#endif
//#if __ExGames__
	pCSave.ChatBanTime = ExUser[aIndex].ChatBanTime;
//#endif
//#if __MuRain__
	pCSave.NoobTime = ExUser[aIndex].NoobTime;
//#endif
//#ifdef _MuReal_
	pCSave.OffAfk = ExUser[aIndex].OffAfk;
	pCSave.VipTime = ExUser[aIndex].VipTime;
	pCSave.CR = ExUser[aIndex].CR;
//#endif
...
//#endif
//#if(BM_QUEST_SYSTEM_)
	pCSave.BMQuest_num = ExUser[aIndex].BMQuest_num;
	pCSave.BMQuest_kill = ExUser[aIndex].BMQuest_kill;
	pCSave.BMQuest_start = ExUser[aIndex].BMQuest_start;
	pCSave.BMDamageReflect = ExUser[aIndex].BMDamageReflect;
	pCSave.BMDamage = ExUser[aIndex].BMDamage;
	pCSave.BMDefense = ExUser[aIndex].BMDefense;
	pCSave.BMCriticalDamage = ExUser[aIndex].BMCriticalDamage;
	pCSave.BMExcelentDamage = ExUser[aIndex].BMExcelentDamage;
	pCSave.BMLife = ExUser[aIndex].BMLife;
//#endif

	pCSave.ChaosBank = lpObj->ChaosBank;
	pCSave.BlessBank = lpObj->BlessBank;
	pCSave.SoulBank = lpObj->SoulBank;
	pCSave.LifeBank = lpObj->LifeBank;

	pCSave.MTDate = lpObj->MTDate;
...
void GDSetWarehouseList(int aIndex)
{
	...
	pMsg.vNum = gObj[aIndex].MultiVault;
...
void ItemSerialCreateSend(int aIndex, BYTE MapNumber, BYTE x, BYTE y, int type, BYTE level, BYTE dur, BYTE Op1, BYTE Op2, BYTE Op3, int LootIndex, BYTE NewOption, BYTE SetOption)
{
	...
	{
		if(!NewgItemDropManager.BlockItemDrop(aIndex,type,MapNumber)) return;
	}
#endif

#if(ITEM_NO_NAME_DROP_FIX==TRUE)
	if(type == -1)
	{
		LogAddTD("[ItemSerialCreateSend] No Name Item Drop Block");
		return;
	}
#endif
...
void ItemSerialCreateRecv(SDHP_ITEMCREATERECV * lpMsg)
{
	...
	if ( lpMsg->MapNumber == (BYTE)-1 || lpMsg->MapNumber == (BYTE)-2 )
	{
		...
		if(GrandHero.SockeetItem(lpMsg->Type) == TRUE)
		{
			BYTE Sockeet[5] = {0xFE,0xFE,0xFE,0xFE,0xFE};
			NewItem.Convert(lpMsg->Type,lpMsg->Op1, lpMsg->Op2, lpMsg->Op3,lpMsg->NewOption, lpMsg->SetOption,0, Sockeet, 0xFF, CURRENT_DB_VERSION);
		}
		else
		{
			NewItem.Convert(lpMsg->Type,lpMsg->Op1, lpMsg->Op2, lpMsg->Op3,lpMsg->NewOption, lpMsg->SetOption,0, 0, 0xFF, CURRENT_DB_VERSION);
		}
#else
		NewItem.Convert(lpMsg->Type,lpMsg->Op1, lpMsg->Op2, lpMsg->Op3,lpMsg->NewOption, lpMsg->SetOption,0, 0, 0xFF, CURRENT_DB_VERSION);
#endif
	...
	else if ( lpMsg->MapNumber == 236 )
	{
		if ( gObj[lpMsg->aIndex].Connected > PLAYER_CONNECTED )
		{
			...
			if(lpMsg->x == 1 && lpMsg->y == 1)
			{
				pCreateItem.m_Number = 1;//lpMsg->m_Number;
			}

#if CASH_SHOP_ADDITIONAL == 1
			if ( g_SocketOption.CheckItemType(ITEMGET(iItemType, iItemIndex)) == 1 )
			{
				BYTE CS_Item[5] = {0xFE,0xFE,0xFE,0xFE,0xFE};
				pCreateItem.Convert(ITEMGET(iItemType, iItemIndex), lpMsg->Op1, lpMsg->Op2, lpMsg->Op3, lpMsg->NewOption, lpMsg->SetOption, 0, CS_Item, 0xFF, 3);
			}
			else
			{
...
void GS_GDReqSiegeDateChange(int iMapSvrGroup, int aIndex,WORD wStartYear, BYTE btStartMonth, BYTE btStartDay, WORD wEndYear, BYTE btEndMonth, BYTE btEndDay)
{
	...
	if(MSSQL2008)
	{
		pMsg.btStartMonth = btStartDay;
		pMsg.btStartDay = btStartMonth;
	}
	else
	{
		pMsg.btStartMonth = btStartMonth;
...
void DGSummonerStateUpdatedSend(LPPMSG_ANS_SUMMONER_CREATE lpMsg)
{
	...
	gObj[aIndex].Summoner = 1;	//Fix Summoner
	...
	pMsg.btResult = 0;
	pMsg.IsSummonerEnable = 1;//lpMsg->IsSummonerEnable;
...
void DGSummonerStateRecv(LPPMSG_ANS_SUMMONER_STATUS lpMsg)
{
	...
	pMsg.btResult = 1;
	pMsg.IsSummonerEnable = 1;//lpMsg->btResult;

	DataSend(aIndex, (LPBYTE)&pMsg, sizeof(pMsg));
}

struct PMSG_REQ_WCOIN_INFO_LOAD
{
	PBMSG_HEAD h;
	int iUserIndex;
	char Accaunt[MAX_ACCOUNT_LEN+1];
	int Count;
	int Type;
};

void GDRequestWcoinInfoLoad (LPOBJ lpObj,int Count,int Type)
{
	PMSG_REQ_WCOIN_INFO_LOAD pMsg;

	pMsg.h.set((LPBYTE)&pMsg,0xD6,sizeof(pMsg));

	pMsg.iUserIndex = lpObj->m_Index;

	memset(pMsg.Accaunt, 0, sizeof(pMsg.Accaunt));
	memcpy(pMsg.Accaunt, lpObj->AccountID, MAX_ACCOUNT_LEN);
	...
	LogAddTD("[%s][%s] SEND Packet OffTrade To DS",lpObj->AccountID,lpObj->Name);
}

struct PMSG_ANS_WCOIN_INFO_LOAD 
{
	PBMSG_HEAD h;
	BYTE Result;
	int Index;
...
void DGRequestWcoinInfoLoad(LPBYTE aRecv)
{
	if(aRecv == NULL)
	{
		return;
	...
	PMSG_ANS_WCOIN_INFO_LOAD * pRecv = (PMSG_ANS_WCOIN_INFO_LOAD *)aRecv;
	...
	if(pRecv->Result == 0)
	{
		LogAddTD("[%s][%s] OFFTRADE %d", lpObj->AccountID,lpObj->Name,pRecv->Count);
	}
	else
	{
		if(pRecv->Type == 1)
		{
			lpObj->m_wCashPoint += pRecv->Count;
		}
		else if(pRecv->Type == 2)
		{
			lpObj->m_wCashPoint -= pRecv->Count;
		}
	}
