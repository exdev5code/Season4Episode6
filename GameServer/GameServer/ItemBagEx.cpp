		...
		if ( Token == 1 )
		{
			...
			else if ( st == 2 )
			{
				while ( true )
				{
					...
					Token = GetToken();
					this->BagObject[n].m_minLevel = (BYTE)TokenNumber;

					Token = GetToken();
					this->BagObject[n].m_maxLevel = (BYTE)TokenNumber;

					Token = GetToken();
					this->BagObject[n].m_isskill = (BYTE)TokenNumber;

					Token = GetToken();
					this->BagObject[n].m_isluck = (BYTE)TokenNumber;

					Token = GetToken();
					this->BagObject[n].m_isoption = (BYTE)TokenNumber;

					Token = GetToken();
					this->BagObject[n].m_isexitem = (BYTE)TokenNumber;
...
BOOL CItemBagEx::DropEventItem(int aIndex)
{
	...
	if ( (rand()%1000) < this->m_iEventItemDropRate )
	{
		...
		ItemSerialCreateSend(lpObj->m_Index, (BYTE)lpObj->MapNumber, (BYTE)x ,(BYTE)y, ei, (BYTE)eil, (BYTE)dur, 0, 0, 0, thdu, 0, 0);
...
BOOL CItemBagEx::DropItem(int aIndex)
{
	...
	if ( this->GetBagCount() > 0 )
	{
		if ( (rand()%100) < this->m_iItemDropRate )
		{
			...
			ItemSerialCreateSend(lpObj->m_Index, (BYTE)lpObj->MapNumber, (BYTE)x, (BYTE)y, type, (BYTE)level, (BYTE)dur, (BYTE)Option1, (BYTE)Option2, (BYTE)Option3, lpObj->m_Index, (BYTE)ExOption, 0);
	...
	if ( this->GetBagCount() > 0 )
	{
		...
		else
		{
			...
			ItemSerialCreateSend(lpObj->m_Index, (BYTE)lpObj->MapNumber, (BYTE)x, (BYTE)y, type, (BYTE)level, (BYTE)dur,
				(BYTE)Option1, (BYTE)Option2, (BYTE)Option3, lpObj->m_Index, (BYTE)ExOption, 0);
...
BOOL CItemBagEx::DropHallowinEventItem(LPOBJ lpObj)
{
	...
	if ( iItemBagCount > 0 )
	{
		...
		ItemSerialCreateSend(lpObj->m_Index, (BYTE)lpObj->MapNumber, (BYTE)X, (BYTE)Y,
			iType, (BYTE)iLevel, (BYTE)iDuration, 0, 0, 0, lpObj->m_Index, 0, 0);
...
BOOL CItemBagEx::DropKundunEventItem(int aIndex, BYTE btMapNumber, BYTE cX, BYTE cY)
{
	...
	if ( this->GetBagCount() > 0 )
	{
		if ( true )
		{
			...
			ItemSerialCreateSend(lpObj->m_Index, (BYTE)btMapNumber, (BYTE)x, (BYTE)y, type, (BYTE)level, (BYTE)dur, (BYTE)Option1, (BYTE)Option2, (BYTE)Option3, lpObj->m_Index, (BYTE)ExOption, 0);
...
BOOL CItemBagEx::DropCrywolfDarkElfItem(int aIndex, BYTE btMapNumber, BYTE cX, BYTE cY)
{
	...
	if ( this->GetBagCount() > 0 )
	{
		...
		else
		{
			...
			ItemSerialCreateSend(lpObj->m_Index, (BYTE)btMapNumber, (BYTE)X, (BYTE)Y, iType, (BYTE)iLevel, (BYTE)fDur,
				(BYTE)iOption1, (BYTE)iOption2, (BYTE)iOption3, lpObj->m_Index, (BYTE)iExOption, 0);
