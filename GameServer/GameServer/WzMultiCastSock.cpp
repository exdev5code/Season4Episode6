...
void CWzMultiCastSock::SaveLastError(int iErrorNo)
{
	...
	FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, this->m_uLastErrorNum, LANG_NEUTRAL, this->m_szLastErrorMessage, sizeof(this->m_szLastErrorMessage), NULL);
...
unsigned int __stdcall CWzMultiCastSock::RecvThreadProc(void * lpParam)
{
	...
	while ( true )
	{
		int iRecvLen = recvfrom(lpCMSock->m_skSocket, cBUFFER, sizeof(cBUFFER), 0, (::sockaddr *)&sockaddr, &isockaddr_len);
		...
		switch ( (BYTE)cBUFFER[0] )
		{
			case 0xC1:
				...
				{
					lpCMSock->m_lpfnCALLBACK_RECVNOTIFY(lpCMSock, sockaddr.sin_addr.S_un.S_addr, btHeadCode, btSubCode, cBUFFER, iRecvLen);
				}
				break;
			case 0xC2:
