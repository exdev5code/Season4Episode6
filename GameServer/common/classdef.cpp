...
void classdef::Init()
{
	...
	this->SetCharacter( 5, 21, 21, 18, 23, 70.0, 40.0, 1.0, 1.5, 2.0, 1.5, 0);
}
...
void classdef::SetEquipment(int Class)
{
	...
	if ( Class != 0 )
	{
		if ( Class == CLASS_DARKLORD )
		{
			...
			this->DefClass[Class].Equipment[0].Convert(right_type, 0,0,0,0,0,0,NULL, 0xFF, CURRENT_DB_VERSION);
			this->DefClass[Class].Equipment[1].Convert(def_type, 0,0,0,0,0,0,NULL, 0xFF, CURRENT_DB_VERSION);
		}
		...
		else if( Class == CLASS_SUMMONER )
		{
			...
			DefClass[Class].Equipment[0].Convert(def_type, 0,0,0,0,0,0,NULL, 0xFF, CURRENT_DB_VERSION);
		}
		else
		{
			this->DefClass[Class].Equipment[0].m_Level = 0;
			this->DefClass[Class].Equipment[0].Convert(def_type, 0,0,0,0,0,0,NULL, 0xFF, CURRENT_DB_VERSION);
		}
	}
	...
	if ( this->m_BabubBanJiCreate == TRUE )
	{
		...
		this->DefClass[Class].Equipment[12].Convert(def_type, 0,0,0,0,0,0,NULL, 0xFF, CURRENT_DB_VERSION);
		
		def_type = ItemGetNumberMake(13, 20);
		this->DefClass[Class].Equipment[13].m_Level = 2;
		this->DefClass[Class].Equipment[13].Convert(def_type, 0,0,0,0,0,0,NULL, 0xFF, CURRENT_DB_VERSION);
	}
