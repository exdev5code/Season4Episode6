...
void InitScreenGlow(void)
{
	 ...
	 // VP LOADING
     if (!LoadProgram(&g_cgVP_GlowDarken, g_cgVertProfile, "Data/ExTeam/cgprograms/glow_darken_vp.cg"))
          return;

     if (!LoadProgram(&g_cgVP_GlowBlur, g_cgVertProfile, "Data/ExTeam/cgprograms/glow_blur_vp.cg"))
          return;

     if (!LoadProgram(&g_cgVP_GlowCombine, g_cgVertProfile, "Data/ExTeam/cgprograms/glow_combine_vp.cg"))
          return;
	 ...
	 // FP LOADING
     if (!LoadProgram(&g_cgFP_GlowDarken, g_cgFragProfile, "Data/ExTeam/cgprograms/glow_darken_fp.cg"))
          return;

     if (!LoadProgram(&g_cgFP_GlowBlur, g_cgFragProfile, "Data/ExTeam/cgprograms/glow_blur_fp.cg"))
          return;

     if (!LoadProgram(&g_cgFP_GlowCombine, g_cgFragProfile, "Data/ExTeam/cgprograms/glow_combine_fp.cg"))
          return;
}
...
void Graphics::Load()
{
	...
	SetOp((LPVOID)gSelectEnter, (LPVOID)gSelectASM00, ASM::JMP);
	SetOp((LPVOID)gGraphicsEnter, (LPVOID)gGraphicsASM00, ASM::JMP);
}
